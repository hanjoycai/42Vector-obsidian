...
> **测试芯片**：自研芯片 X1 vs NVIDIA A100 80GB
> **测试模型**：Qwen2.5-72B-Instruct
...
> **目的**：本页是领导唯一会仔细看的一页。必须在 1 页内回答"买不买"的问题。

### 1.1 三句话结论

> [!important] 核心结论
> 1. **性能**：自研芯片 X1 在 AI Coding Agent 场景下 MLPerf 归一化吞吐量 = A100 的 **1.2×**，首轮 TTFT **135ms**（A100: 148ms），加权 TPOT **13.8ms/tok**（A100: 14.2ms/tok）
...
| 风险项 | 级别 | 说明 | 建议措施 |
| --- | --- | --- | --- |
| Agent 任务成功率差距 | 🟡 中 | X1 比 A100 低 0.6pp，主要因 FP8 量化下工具调用正确率下降 | 建议测试 BF16 精度部署 |
| P99 TTFT 尾部延迟 | 🟡 中 | X1 在 16+ 并发时 P99 TTFT 达 480ms，接近 500ms SLA 红线 | 优化 KV Cache 预分配策略 |
...
> **本章说明**：AaaS 验证平台的指标体系不是简单地"多测几个数"，而是一套面向不同受众、从体验到根因层层递进的三层架构，服务于一个核心目的——**用数据驱动芯片采购决策**。
...
| 配置项 | 自研芯片 X1 | 基准：NVIDIA A100 80GB |
| --- | --- | --- |
| **芯片型号** | X1-80G Rev.2 | A100-SXM4-80GB |
...
| 配置项 | 自研芯片 X1 | 基准 A100 |
| --- | --- | --- |
| **驱动版本** | X1-Driver v2.4.1 | NVIDIA 535.129.03 |
| **推理引擎** | vLLM v0.7.2 (X1-fork) | vLLM v0.7.2 |
| **模型** | Qwen2.5-72B-Instruct | Qwen2.5-72B-Instruct |
| **量化模式** | FP8 (W8A8) | FP16 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| | **X1** | **A100** | **X1** | **A100** | **X1** | **A100** | **X1** | **A100** |
...
**【稳定性分布验证 —— 自研 X1 vs A100】**

| 指标 | | P50 | P75 | P90 | P99 | max | SLO_P90 | 判定 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **TTFT (ms)** | X1 | 95 | 120 | 142 | 380 | 520 | ≤200 | 🟢 |
| | A100 | 88 | 110 | 135 | 340 | 480 | ≤200 | 🟢 |
| **TPOT (ms/tok)** | X1 | 12.5 | 13.8 | 15.2 | 18.5 | 22.1 | ≤20 | 🟢 |
| | A100 | 12.0 | 13.2 | 14.6 | 17.2 | 20.8 | ≤20 | 🟢 |
| **Decode 延迟 (ms)** | X1 | 72 | 85 | 96 | 125 | 168 | ≤120 | 🟢 |
| | A100 | 68 | 80 | 92 | 118 | 155 | ≤120 | 🟢 |
| **E2E 推理延迟 (ms)** | X1 | 165 | 185 | 210 | 280 | 350 | ≤250 | 🟢 |
| | A100 | 155 | 175 | 198 | 260 | 330 | ≤250 | 🟢 |
| **生成速度 (tok/s)** | X1 | 72 | 68 | 64 | 52 | 42 | ≥55 | 🟢 |
...
| 场景 | 自研 X1 (tok/s) | A100 (tok/s) | 归一化（A100=1.0） | 判定 |
| --- | --- | --- | --- | --- |
...
> **解读**：Server 归一化 1.20× 意味着"自研 X1 在有延迟约束的生产场景下吞吐量是 A100 的 1.2 倍"，Interactive 场景（最接近 Agent 实时交互）归一化 1.12×。

---

## 六、诊断指标 —— "慢在哪里？为什么慢？稳不稳？"

> **价值定位**：当体验指标不达标时，诊断指标帮助工程团队快速定位瓶颈。
...
> **解读**：在自研 X1 上用 AI Coding Agent 修一个 Bug 的全链路成本约 ¥0.10，比 A100 便宜 32%。

#### 📊 图表 C1：跨芯片 ¥/M-token 对比柱状图

> 自研 X1 / A100 / H100 / 公有云 API 对比
> **我们比 A100 和公有云便宜多少**
...
> 自研 X1 / A100 / H100 / 昇腾 的 token/s/W 对比
...
### 8.2 SWE-bench Lite 代表性 Case 列表
...
### 8.3 HumanEval 代表性 Case 列表

| Case ID | 函数名 | 任务描述 | 难度 | 预期轮数 |
| --- | --- | --- | --- | --- |
| HE-001 | has_close_elements | 判断列表中是否有距离小于阈值的元素对 | 简单 | 2-3 |
| HE-023 | strlen | 计算字符串长度 | 简单 | 1-2 |
| HE-045 | triangle_area | 根据三边长计算三角形面积 | 简单 | 2-3 |
| HE-067 | fruit_distribution | 解析字符串中的水果数量分配 | 中等 | 3-4 |
| HE-089 | encrypt | 实现字母表移位加密 | 中等 | 3-4 |
| HE-112 | reverse_delete | 删除指定字符后判断是否回文 | 中等 | 3-5 |
| HE-134 | check_if_last_char_is_letter | 检查字符串最后一个字符是否为字母 | 简单 | 1-2 |
| HE-156 | int_to_mini_roman | 整数转罗马数字 | 困难 | 4-6 |

### 8.4 内部 Coding Benchmark Case 列表
...
### 8.5 GAIA 评估集 Case 列表

| Case ID | Level | 任务描述 | 涉及工具 | 预期轮数 |
| --- | --- | --- | --- | --- |
| GAIA-L1-01 | Level 1 | 查询并计算特定城市天气统计 | WebSearch, Calculator | 3-4 |
| GAIA-L1-15 | Level 1 | 从 PDF 中提取并汇总财务数据 | FileRead, Calculator | 3-5 |
| GAIA-L2-08 | Level 2 | 跨多个数据源进行信息整合分析 | WebSearch, FileRead, Code | 6-8 |
| GAIA-L2-22 | Level 2 | 根据多步骤指令完成数据处理 | Code, FileWrite, Verify | 5-7 |
| GAIA-L3-05 | Level 3 | 复杂多约束条件下的方案规划 | WebSearch, Code, Analyze | 10-14 |
| GAIA-L3-12 | Level 3 | 端到端研究报告生成 | WebSearch, FileRead, Write | 12-18 |
...
> [!success] 总体结论
> 自研芯片 X1 在 AI Coding Agent 场景下**整体性能达到 A100 的 1.05-1.20 倍**，**成本降低 35%**，已通过 **7 天长稳验证和 5 个模型适配测试**，**推荐进入下一阶段 KA 客户 POC 验证**。
...
| 🟡 中 | **优化连续批处理参数**：max_batch_size 和 chunk_size 针对 X1 内存架构调优 | 推理引擎团队 | 并发扩展效率预计提升 5-8pp |
| 🟡 中 | **验证 Prefill/Decode 分离部署**：X1 的高 HBM 带宽适合 Decode 专用节点 | 架构团队 | 吞吐量预计提升 20-30% |
...
| 阶段 | 内容 | 时间 | 目标 |
| --- | --- | --- | --- |
| Phase 2 | KA 客户真实业务场景 POC | 2026-05 | 验证 X1 在客户自有 Agent 上的表现 |
| Phase 3 | 多模型混合部署测试 | 2026-06 | 验证 7B 路由 + 72B 执行模型共置效率 |
| Phase 4 | 下一代 X2 芯片基准测试 | 2026-Q3 | 建立 X2 vs X1 vs A100 三方对比基线 |
...
| 配置类别 | 配置项 | 自研 X1 | 基准 A100 |
| --- | --- | --- | --- |
| **芯片** | 型号 | X1-80G Rev.2 | A100-SXM4-80GB |
| | 显存 | 80 GB HBM2e | 80 GB HBM2e |
| | 驱动版本 | X1-Driver v2.4.1 | NVIDIA 535.129.03 |
| | 固件版本 | X1-FW v1.8.0 | — |
| **推理引擎** | 引擎 | vLLM v0.7.2 (X1-fork) | vLLM v0.7.2 |
| | tensor_parallel_size | 4 | 4 |
| | max_model_len | 32768 | 32768 |
| | max_num_batched_tokens | 8192 | 8192 |
| | enable_prefix_caching | true | true |
| | gpu_memory_utilization | 0.92 | 0.90 |
| **模型** | 名称 | Qwen2.5-72B-Instruct | Qwen2.5-72B-Instruct |
| | 量化模式 | FP8 (W8A8) | FP16 |
