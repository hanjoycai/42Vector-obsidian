...
| 指标类型 | 关注点   | 验证名称                              | 验证价值       |
| ---- | ----- | --------------------------------- | ---------- |
| 体验指标 | 用户体验  | 4.6.4.x.3 单一任务端到端测试               | 好不好用       |
|      | 系统吞吐  | 4.6.4.x.4 并发任务端到端测试-综合性能测试        |            |
| 诊断指标 | 链路归因  | 4.6.4.x.5 过程链路测试指标（偏向业务逻辑拆解）      | 如果不好，问题出在哪 |
|      | 底层定位  | 4.6.4.x.6 Agent 任务运行时指标（偏向底层资源拆解） |            |
| 稳定指标 | 稳定性抖动 | 4.6.4.x.7 百分位性能指标测试               | 稳不稳        |
| 成本指标 | 商业成本  | 4.6.4.x.8 单任务Token消耗成本测试          | 赚不赚        |
