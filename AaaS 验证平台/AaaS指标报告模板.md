...
> 稳定：集群规模下，连续运行的稳定性如何？性能百分位稳定性如何？
...
| 指标类型 | 关注点   | 验证名称                    | 验证价值       |
| ---- | ----- | ----------------------- | ---------- |
| 体验指标 | 用户体验  | 单一任务端到端测试               | 好不好用       |
|      | 系统吞吐  | 并发压力测试                  |            |
| 诊断指标 | 链路归因  | 过程链路测试指标（偏向业务逻辑拆解）      | 如果不好，问题出在哪 |
|      | 底层定位  | Agent 任务运行时指标（偏向底层资源拆解） |            |
| 稳定指标 | 稳定性抖动 | 百分位性能指标测试               | 稳不稳        |
| 成本指标 | 商业成本  | 单任务Token消耗成本测试          | 赚不赚        |
...
核心价值：通过体验、稳定、诊断、成本四个维度指标的验证，量化验证当前自研芯片、集群规模和相关配置项下：**是否精准匹配"某个行业场景的工作负载特征"**。匹配度高则资源利用率高、成本最优。

| 价值维度          | 关键问题                       | 价值说明                                                                                                                                                                                                                       | 核心观测指标                                                                   |
| ------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **场景准入评估**    | 这款芯片能不能跑这个 Agent 场景？       | 通过单任务基线测试验证芯片在目标 Agent 场景下的基础可用性：TTFT/TPOT 是否满足实时交互体验阈值、任务完成率是否 ≥ 99%、FP8 量化下精度损失是否可接受。**准入不通过则直接排除**，避免后续无效投入                                                                                                             | 首轮 TTFT · 末轮 TTFT · 任务完成率 · 决策准确率 · 工具调用正确率                              |
| **调度策略选型**    | 用什么调度策略跑这个场景最高效？           | 同一芯片 + 同一 Agent 场景，对比不同推理引擎调度策略（Continuous Batching vs PD 分离 vs Chunked Prefill）的表现差异。通过排队时间、抢占次数、KV Cache 使用率峰值三项关键诊断指标，**选出最优调度配置**                                                                                      | 平均排队等待 · 被抢占请求数 · KV Cache 峰值 · Prefix Cache 命中率 · 并发扩展效率                |
| **集群资源配比优化**  | Prefill 和 Decode 该分配几比几的卡？ | 基于推理延迟拆解（Prefill 占比 vs Decode 占比）和硬件利用率数据（MFU vs MBU），**量化指导集群 Prefill/Decode 节点配比**。Prefill 占比高的场景（长上下文输入）需更多计算密集型节点；Decode 占比高的场景（长输出生成）需更多带宽密集型节点                                                                       | Prefill 时延占比 · Decode 时延占比 · MFU · MBU · GPU 利用率 · 互联带宽利用率               |
| **算力与负载匹配验证** | 芯片的算力结构与这个场景的负载特征匹配吗？      | 不同行业场景的工作负载完全不同——AI Coding 是多轮长上下文 + 频繁工具调用（Prefill-heavy），视频理解是大 batch 短输出（Compute-heavy），长文摘要是单轮超长输入（Memory-heavy）。通过 **Prefill/Decode 时间占比、GPU 空闲占比、KV Cache 使用模式、Prefix Cache 效率、并发扩展效率**五个维度，量化验证匹配度。匹配度高则资源利用率高、成本最优 | Prefill/Decode 时间占比 · GPU 空闲率 · KV Cache 使用率 · Prefix Cache 命中率 · 并发扩展效率 |
| **竞品差距量化**    | 和 A800/H100 差多少？差在哪？       | 在**完全相同的评测集、Agent 框架和模型配置**下，对自研芯片和基准芯片进行 A/B 对比。通过四层指标体系逐层对照：体验层看差距大小、稳定层看 SLO 达标率、诊断层看差距根因（算力/带宽/调度/驱动）、成本层看 TCO 优势是否覆盖性能差距。**最终输出综合结论：Pass / Conditional Pass / Fail**                                                  | ¥/M-token 对比 · 每瓦吞吐量 · 节点 ROI                                            |
...
数据说明：请求和轮数-取同一个case跑多次的中位数（取某一次结果）
...
> **一眼看出时间花在哪了**
![image.png](https://42notion.oss-cn-shenzhen.aliyuncs.com/book/20260424201126395.png)

|                                     |                         |                                         |
| ----------------------------------- | ----------------------- | --------------------------------------- |
| **优化 Decode 速度**                    | 深紫色占 76%，是绝对主导          | 如果 TPOT 降低 20%，任务时长减少 ~12s              |
| **优化 R5 轮的生成效率**                    | 单轮 31s 占全任务 40%         | 专项优化这一轮（如推测解码 speculative decoding）收益最大 |
| **优化 Prefill（提高 Prefix Cache 命中率）** | 浅紫色合计仅 5.1s，占 6.5%      | 收益上限有限，但长上下文场景（R7-R8）仍有价值               |
| **P/D 分离部署**                        | Prefill 和 Decode 需求截然不同 | 分离后 Decode 节点专注高 HBM 带宽利用，可提升 Decode 效率 |
...
【并发说明：10 Agent 开启 10 个会话，10 个会话互相隔离同时运行】
...
| ------------------ | ------- | ---- | ---- | ---- | ---- | ---- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TTFT (ms)**      | NGU800P | 95   | 120  | 142  | 380  | 520  | ≤200    | **TTFT 百分位 = 所有请求 TTFT 升序排列后取对应分位点**<br><br>即：$P_N = sorted(TTFT)[⌊N\% \times count⌋]$<br><br>其中：<br>- **TTFT**：单次请求从发送到首 token 返回的时间 = T(first_token) − T(request_sent)<br>- **count**：80 次调用/Case × Case 总数<br>- 单位：**ms**<br><br>**1. P90=142ms 表示 90% 请求首 token 在 142ms 内返回**——直接对标客户 SLO 阈值。<br><br>**2. P99/P50 = 380/95 = 4.0×**——比值越大说明尾部抖动越严重，>5× 需告警。 |
|                    | A800    | 88   | 110  | 135  | 340  | 480  | ≤200    |                                                                                                                                                                                                                                                                                                                                                                 |
| **TPOT (ms/tok)**  | NGU800P | 12.5 | 13.8 | 15.2 | 18.5 | 22.1 | ≤20     | **TPOT 百分位 = 所有请求 TPOT 升序排列后取对应分位点**<br><br>即：$P_N = sorted(TPOT)[⌊N\% \times count⌋]$<br><br>其中：<br>- **TPOT**：每 token 生成延迟 = (T(last_token) − T(first_token)) / (output_tokens − 1)<br>- 单位：**ms/token**<br><br>**1. P99=18.5ms/tok → 最差速度 ≈ 54 tok/s**——接近用户感知"慢速打字"阈值 50 tok/s。<br><br>**2. P99/P50 = 1.48×**——Decode 阶段稳定性较好，抖动可控。                         |
|                    | A800    | 12.0 | 13.2 | 14.6 | 17.2 | 20.8 | ≤20     |                                                                                                                                                                                                                                                                                                                                                                 |
| **Decode 延迟 (ms)** | NGU800P | 72   | 85   | 96   | 125  | 168  | ≤120    | **Decode 延迟百分位 = 所有请求 Decode 阶段耗时升序排列后取对应分位点**<br><br>即：$P_N = sorted(T_{decode})[⌊N\% \times count⌋]$<br><br>其中：<br>- **T_decode** = T(last_token) − T(first_token)<br>- 单位：**ms**<br><br>**1. Decode 延迟与 output_tokens 正相关**——P99/P50 = 1.74× 受不同 Case 输出长度差异影响。<br><br>**2. P99=125ms 达标但接近 SLO=120ms**——高并发下可能超标。                                           |
|                    | A800    | 68   | 80   | 92   | 118  | 155  | ≤120    |                                                                                                                                                                                                                                                                                                                                                                 |
| **E2E 推理延迟 (ms)**  | NGU800P | 165  | 185  | 210  | 280  | 350  | ≤250    | **E2E 推理延迟百分位 = 所有请求完整推理耗时升序排列后取对应分位点**<br><br>即：$P_N = sorted(T_{e2e})[⌊N\% \times count⌋]$<br><br>其中：<br>- **T_e2e** = 排队 + Prefill + Decode + 后处理<br>- 单位：**ms**<br><br>**1. P90=210ms 达标（SLO=250ms）**——有 40ms 余量，可作为生产 SLA 配置参考值。<br><br>**2. P99=280ms 超 SLO**——极端情况存在长尾，需关注排队和 KV Cache 争用。                                                               |
|                    | A800    | 155  | 175  | 198  | 260  | 330  | ≤250    |                                                                                                                                                                                                                                                                                                                                                                 |
| **请求推理平均时延 (ms)**  | NGU800P | 105  | 182  | 212  | 275  | 345  | ≤250    | **请求推理时延百分位 = 所有请求完整推理耗时（排队+Prefill+Decode+后处理）的分布**<br><br>即：$P_N = sorted(T_{inference})[⌊N\% \times count⌋]$<br><br>其中：<br>- **T_inference**：单次请求从进入推理引擎到结果返回的总耗时<br>- 单位：**ms**<br><br>**1. SLO_P90=250ms 可直接作为生产 SLA 配置参考值**——P90=212ms 达标。<br><br>**2. P99=275ms 超 SLO**——最差 1% 请求超标，需关注长上下文 Prefill 与排队叠加。                                               |
...
| 指标                  |         | P50   | P75   | P90   | P99   | max   | SLO_P90 | 计算说明                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------- | ------- | ----- | ----- | ----- | ----- | ----- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **输入 token 平均长度**   | NGU800P | 1,200 | 1,800 | 2,050 | 2,280 | 2,500 | —       | **输入 token 长度百分位 = 所有请求 input_tokens 升序排列后取对应分位点**<br><br>即：$P_N = sorted(input\_tokens)[⌊N\% \times count⌋]$<br><br>其中：<br>- **input_tokens**：单次请求的输入 token 数（含 system prompt + 历史上下文 + 工具定义）<br>- 单位：**tokens**<br><br>**1. 刻画测试输入负载的长度特征**——客户对标自身负载分布评估验证场景的代表性。<br><br>**2. 两张芯片数据一致**——使用同一评估集，输入负载完全对等。                                                                         |
|                     | A800    | 1,200 | 1,800 | 2,050 | 2,280 | 2,500 | —       |                                                                                                                                                                                                                                                                                                                                                                                        |
| **生成 token 平均长度**   | NGU800P | 320   | 400   | 485   | 540   | 620   | —       | **生成 token 长度百分位 = 所有请求 output_tokens 升序排列后取对应分位点**<br><br>即：$P_N = sorted(output\_tokens)[⌊N\% \times count⌋]$<br><br>其中：<br>- **output_tokens**：单次请求模型实际生成的 token 数（含工具调用指令、代码、文本）<br>- 单位：**tokens**<br><br>**1. 输出长度分布影响 Decode 延迟和成本**——P99=540 token 的请求 Decode 耗时约为 P50=320 的 1.7 倍。<br><br>**2. 结合 TPOT 可推算任务完成时间**——P99 输出 540 tok × 18.5ms/tok ≈ 10s Decode 时间。              |
|                     | A800    | 320   | 400   | 485   | 540   | 620   | —       |                                                                                                                                                                                                                                                                                                                                                                                        |
| **生成字符平均长度**        | NGU800P | 1,600 | 2,500 | 2,850 | 3,150 | 3,800 | —       | **生成字符长度百分位 = 所有请求输出文本字符长度升序排列后取对应分位点**<br><br>即：$P_N = sorted(output\_chars)[⌊N\% \times count⌋]$<br><br>其中：<br>- **output_chars**：模型输出文本的 Unicode 字符数（非 byte）<br>- 单位：**字符**<br><br>**1. 对中文/CJK 语言场景尤为重要**——1 token ≈ 1-2 个中文字，字符数比 token 数更直观。<br><br>**2. 字符/token 比反映 tokenizer 效率**——下方"每 token 平均生成字符数"专项度量。                                                                   |
|                     | A800    | 1,600 | 2,500 | 2,850 | 3,150 | 3,800 | —       |                                                                                                                                                                                                                                                                                                                                                                                        |
| **每 token 平均生成字符数** | NGU800P | 4.8   | 5.0   | 5.2   | 5.5   | 6.2   | —       | **每 token 字符数百分位 = 所有请求的 (output_chars / output_tokens) 升序排列后取对应分位点**<br><br>即：$P_N = sorted(\frac{chars_i}{tokens_i})[⌊N\% \times count⌋]$<br><br>其中：<br>- **chars_i**：第 i 个请求的输出字符数<br>- **tokens_i**：第 i 个请求的输出 token 数<br>- 单位：**字符/token**<br><br>**1. 验证跨芯片 tokenizer 行为一致性**——两张芯片数值完全一致，确认 tokenizer 无差异。<br><br>**2. 中文场景 ≈ 4-6 字符/token**——符合 Qwen/GLM 系列 tokenizer 的编码效率预期。 |
...
| 指标                          |         | P50 | P75  | P90  | P99  | max | SLO_P90 | 计算说明                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------------- | ------- | --- | ---- | ---- | ---- | --- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **tokenizer 平均处理时间 (ms)**   | NGU800P | 6   | 10   | 11.5 | 14.2 | 18  | ≤12     | **tokenizer 处理时间百分位 = 所有请求 tokenizer 编码耗时升序排列后取对应分位点**<br><br>即：$P_N = sorted(T_{tokenizer})[⌊N\% \times count⌋]$<br><br>其中：<br>- **T_tokenizer**：将原始文本编码为 token 序列的 CPU 耗时<br>- 单位：**ms**<br><br>**1. tokenizer 在 CPU 上执行**——验证芯片软件栈不引入额外 tokenization 开销。<br><br>**2. P90=11.5ms 接近 SLO=12ms**——长输入文本场景下可能超标，需关注。                                                        |
|                             | A800    | 5.5 | 9.5  | 11   | 13.8 | 16  | ≤12     |                                                                                                                                                                                                                                                                                                                                                                            |
| **detokenizer 平均处理时间 (ms)** | NGU800P | 4   | 6.2  | 7.5  | 10.2 | 13  | ≤8      | **detokenizer 处理时间百分位 = 所有请求 detokenizer 解码耗时升序排列后取对应分位点**<br><br>即：$P_N = sorted(T_{detokenizer})[⌊N\% \times count⌋]$<br><br>其中：<br>- **T_detokenizer**：将 token ID 序列解码为可读文本的 CPU 耗时<br>- 单位：**ms**<br><br>**1. detokenizer 是后处理管线的一部分**——确保完整管线延迟被充分计量。<br><br>**2. P90=7.5ms 接近 SLO=8ms**——长输出场景可能触发后处理瓶颈。                                                             |
|                             | A800    | 3.8 | 5.8  | 7    | 9.5  | 12  | ≤8      |                                                                                                                                                                                                                                                                                                                                                                            |
| **所有 token 平均模型推理时间 (ms)**  | NGU800P | 65  | 92   | 105  | 128  | 155 | ≤110    | **纯模型推理时间百分位 = 所有请求纯模型前向计算耗时（不含排队/后处理/网络）升序排列后取对应分位点**<br><br>即：$P_N = sorted(T_{forward})[⌊N\% \times count⌋]$<br><br>其中：<br>- **T_forward**：模型前向推理计算耗时 = Prefill + Decode，排除排队和后处理<br>- 单位：**ms**<br><br>**1. 芯片计算能力的 ground-truth 指标**——排除一切非芯片因素，直接衡量芯片推理性能。<br><br>**2. P90=105ms 接近 SLO=110ms**——芯片在长输出请求下的纯计算耗时接近上限。                                                |
|                             | A800    | 62  | 88   | 100  | 125  | 148 | ≤110    |                                                                                                                                                                                                                                                                                                                                                                            |
| **Prefill 阶段 batchsize 均值** | NGU800P | 18  | 28   | 31   | 35   | 42  | —       | **Prefill batchsize 百分位 = 各采样点 Prefill 批量大小升序排列后取对应分位点**<br><br>即：$P_N = sorted(BS_{prefill})[⌊N\% \times count⌋]$<br><br>其中：<br>- **BS_prefill**：推理引擎每批处理的 Prefill 请求数（或 token 数）<br>- 单位：**tokens/batch 或 requests/batch**<br><br>**1. 高批量说明芯片显存和算力支持高效并行 Prefill**——批量越大 Prefill 阶段吞吐越高。<br><br>**2. NGU800P P50=18 < A800 P50=20**——Prefill 批量略低，可能受 KV Cache 预分配策略影响。 |
|                             | A800    | 20  | 30   | 32   | 36   | 44  | —       |                                                                                                                                                                                                                                                                                                                                                                            |
| **Decode 阶段 batchsize 均值**  | NGU800P | 9   | 12   | 14.5 | 18   | 22  | —       | **Decode batchsize 百分位 = 各采样点 Decode 批量大小升序排列后取对应分位点**<br><br>即：$P_N = sorted(BS_{decode})[⌊N\% \times count⌋]$<br><br>其中：<br>- **BS_decode**：连续批处理中每批并行 Decode 的请求数<br>- 单位：**requests/batch**<br><br>**1. Decode 批量直接决定吞吐量上限**——批量越大，HBM 带宽利用率越高。<br><br>**2. NGU800P P50=9 < A800 P50=10**——Decode 批量略低，可能受 KV Cache 显存碎片影响。                                            |
|                             | A800    | 10  | 13   | 15   | 19   | 24  | —       |                                                                                                                                                                                                                                                                                                                                                                            |
...
| **Goodput 率**     | 96.9%         | 95.2%         | +1.7pp | **Goodput 率 = 有效吞吐量 / 输出吞吐量 × 100%**<br><br>即：$GoodputRate = \frac{Goodput}{Throughput_{output}} \times 100\%$<br><br>其中：<br>- **Goodput**：仅成功请求产出的 output token 吞吐量（tokens/s）<br>- **Throughput_output**：全部请求（含失败）的 output token 吞吐量（tokens/s）<br>- 单位：**%**<br><br>**1. Goodput 率 = 系统的有效产出效率**——96.9% 意味着 3.1% 的输出 token 因超时截断、格式错误、被抢占中断等原因成为无效产出，消耗了算力和电力但不产生业务价值。<br><br>**2. Goodput 率 = 成功率的 token 维度映射**——请求级成功率关注"多少请求成功了"，Goodput 率关注"多少 token 产出是有效的"。长输出请求超时截断时，请求级成功率不受影响但 Goodput 率会下降。<br><br>**3. 示例**：Throughput_output = 1,600 tok/s，Goodput = 1,550 tok/s → GoodputRate = 1,550 / 1,600 = 96.9%，差值 50 tok/s 的算力被浪费。 |
...
| 指标            | NGU800P     | A800        | 公有云 (火山引擎)   | 对比                     | 计算说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------- | ----------- | ----------- | ------------ | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| 加速卡成本               | ¥0.005     | ¥0.008     | **加速卡成本 = 加速卡单价 × 卡数 × 任务占用时长 / 加速卡生命周期总时长**<br><br>即：$Cost_{GPU} = \frac{Price_{card} \times N_{cards} \times T_{task}}{L_{life} \times H_{year} \times 3600}$<br><br>其中：<br>- **Price_card**：单张加速卡采购价格（¥）<br>- **N_cards**：任务占用的加速卡数量（TP=4 时为 4 张）<br>- **T_task**：任务耗时（秒）<br>- **L_life**：加速卡使用寿命（年，通常 3-5 年）<br>- **H_year**：年有效运行小时数（7×24 = 8,760h）<br>- 单位：**¥**<br><br>**1. 加速卡成本 ≠ 设备折旧**——设备折旧含整机（CPU/内存/机箱/网卡等），加速卡成本仅计算 GPU 芯片本身的时间分摊，用于精确衡量"芯片价格差异"对单任务成本的影响。<br><br>**2. NGU800P 单卡价格低于 A800**——同等任务下加速卡成本省 37.5%，是总成本优势的核心来源之一。<br><br>**3. 示例**：NGU800P 单卡 ¥X 万 × 4 卡 × 132s / (5年 × 8760h × 3600s) = ¥0.005/任务。 |
...
| **总计（修一个 Bug 的成本）** | **¥0.101** | **¥0.150** | **单任务总成本 = Input 费用 + Output 费用 + 加速卡成本 + 电力费用 + 设备折旧**<br><br>即：$Cost_{task} = Cost_{input} + Cost_{output} + Cost_{GPU} + Cost_{elec} + Cost_{deprec}$<br><br>其中：<br>- 各子项如上所述<br>- 单位：**¥**<br><br>**1. Input token 费用占主导地位（约 66%）**——¥0.067 / ¥0.101 = 66.3%，优化上下文管理是降本的关键。<br><br>**2. NGU800P 单任务成本省 32.7%**——¥0.101 vs ¥0.150，年化到百万级任务可节省约 ¥4.9 万。                                                                                                                                                                                                                                                                       |

> **解读**：在 NGU800P 上用 AI Coding Agent 修一个 Bug 的全链路成本约 ¥0.10（含加速卡分摊），比 A800 便宜 32.7%。其中加速卡成本项（¥0.005 vs ¥0.008）体现了芯片采购价格差异对单任务成本的直接传导。
