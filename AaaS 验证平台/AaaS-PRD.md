...
#### AaaS 推进计划总览
![image.png](https://42notion.oss-cn-shenzhen.aliyuncs.com/book/20260507235259014.png)



---

#### Phase 1：基础能力建设 + A800 先进算力验证（Q2 2026 · 4月）
...
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
|                                                   |                                                                                                                       |
| 子任务1：AaaS验证平台：平台链路方案和技术打通，可观测工具搭建                 | AaaS平台最小化版本开发完成：完成场景执行回归比对、链路回放核心功能开发，前端页面对接以及修复集成问题                                                                  |
| 子任务2：完成OpenClaw首批各类场景CASE接入，打通从场景执行到比对结果的全链路，测试报告 | 1、整理OpenClaw首批场景数据，梳理典型场景，导入场景列表，设置比对基线  <br>2、打通场景执行流程，验证Opik追踪数据集成  <br>3、修复试运行问题，固化执行流程，保证流程稳定可重复                  |
| 子任务3：完善Evaluation & Benchmark构建                   | 1、完善OpenClaw Case 的本地依赖、输入输出、输出评分标准：Evaluation & Benchmark文档  <br>2、验证Evaluation的 10 个Case 集在本地环境中可回放的的推理效果，保证回放稳定可回溯 |
| 子任务4：测试与验证报告输出                                    | 输出OpenClaw场景在单机A800 完整的测试和验证报告                                                                                        |
| 子任务5：实现可控并发批量测试                                   | 1、实现可控并发测试方案，支持并发数配置  <br>2、首批场景批量并发执行，收集全量指标数据，校验数据完整性                                                               |
| 子任务6：集群环境搭建                                       | A800集群环境搭建，依赖外部交付级maas平台，如外部maas解决不了，用两台A800搭建集群，直接用模型接口提供服务                                                          |


---

#### Phase 2：评测能力完善 + 验证场景扩展（Q3 2026 · 5 月）

基于OpenClaw 场景，输出 A800集群测试报告。基于 Claude Code 开源源码和 Coding 任务的BenchMark，调研Coding Agent链路场景并完成技术验证。完成AaaS平台实验比对功能的上线测试。

| Sprint                             | 时间                                                                                                       |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------- |
| 子任务1：Claude Code 开源版本地化部署          | 基于开源版原生代码 Claude Code 完成在虚拟机的部署，支持调用A800本地模型，为后期真实还原Coding Agent 的验证测试构建基础环境                             |
| 子任务2：Claude Code 集成监测工具测试          | 1、深入研究 Claude Code 的链路组件，完成从测试到集成链路的方案设计  <br>2、打通场景执行流程，验证Opik、LiteLLM追踪数据集成，调研LangFuse 集成的可行性          |
| 子任务3：Coding 评测集验证：Terminal-Bench跑通 | 1、产品侧调研Coding 评测集相关业务，从仓库级、终端工具、函数级、 编辑 /多编程语言能力等维度的评测集和场景评分标准  <br>2、验证Terminal-Bench的 Case 集在本地环境中评分效果 |
| 子任务4：OpenClaw 场景， 输出 A800集群测试报告    | 1、完善AaaS指标报告模板-单机/集群/并发详细的指标报告与可视化报表  <br>2、输出OpenClaw场景在A800集群完整的测试和验证报告                                |
| 子任务5：AaaS 平台实验比对功能                 | 1、产品侧完成AaaS平台正式版的需求设计、前端原型验证（增加coding agent评测集管理和指标可视化比对）  <br>2、完成 AaaS平台-实验比对功能的开发、测试、指标验证             |
...
| 功能     | 详细说明                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 交互图 |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| 模型列表   | **功能概述：**展示所有已注册模型<br><br>**列表字段：**<br>1. 模型名称<br>2. 模型提供方（内部 MaaS / 外部 MaaS / 自部署）<br>3. 模型类型（对话 / 补全 / Embedding / 多模态）<br>4. 部署状态（可用 / 不可用 / 维护中）<br>5. 关联 Agent 数<br>6. 最近调用时间<br>7. 接入方式<br><br>**筛选与操作：**<br>1. 支持按提供方、类型、状态多维筛选<br>2. 支持模型名称模糊搜索<br>3. 状态异常模型红色高亮提示<br>4. 每行支持快捷操作：测试连通性、查看调用统计、编辑、停用                                                                                                                                                     |     |
| 注册新模型  | **功能概述：**配置并注册新模型至平台<br><br>**配置项：**<br>1. MaaS 平台选择（内部 MaaS / 火山引擎 / 阿里云百炼 / OpenAI Compatible / 自定义）<br>2. API Endpoint URL（必填）<br>3. API Key / Access Token（加密存储）<br>4. 接入协议（OpenAI Compatible / 自定义 Header）<br>5. LiteLLM 路由标识<br><br>**模型能力声明：**<br>1. 最大上下文长度<br>2. 是否支持 Function Call / Streaming / Vision<br>3. 量化模式（FP32 / FP16 / INT8 / INT4）<br><br>**部署环境标签：**<br>1. 芯片型号（A800 / NGU800P / X6000s）<br>2. 集群标识<br>3. 节点数<br><br>**交互说明：**连通性测试通过后方可保存 |     |
| 模型详情   | **功能概述：**展示模型完整配置信息与当前运行状态<br><br>**调用统计 Tab：**<br>1. 近 24h / 7d / 30d 调用次数<br>2. 平均延迟<br>3. 错误率<br>4. Token 消耗趋势<br><br>**关联资源 Tab：**<br>1. 绑定该模型的 Agent 列表<br>2. 引用该模型作为 Judge 的评估器列表<br><br>**变更记录 Tab：**<br>1. 配置修改历史<br>2. 支持对比不同版本配置差异                                                                                                                                                                                                                     |     |
| 参数预设模板 | **功能概述：**为模型配置可复用参数预设<br><br>**可配置参数：**<br>1. Temperature<br>2. Top-P<br>3. Top-K<br>4. Max Tokens<br>5. Stop Sequences<br>6. Frequency Penalty<br>7. Presence Penalty<br><br>**系统内置默认模板：**<br>1. 「精确模式」（T=0）<br>2. 「创意模式」（T=0.9）<br>3. 「平衡模式」（T=0.5）<br><br>**交互说明：**创建实验时可直接选择预设或在预设基础上微调<br><br>**列表字段：**<br>1. 模板名称<br>2. 关联模型<br>3. Temperature<br>4. Max Tokens<br>5. 创建人<br>6. 关联实验数                                                                      |     |
| 模型分组   | **功能概述：**按芯片环境 / 模型系列分组管理<br><br>**分组用途：**<br>1. 批量筛选<br>2. 实验中快速选择一组模型横向比对<br><br>**交互说明：**一个模型可属于多个分组                                                                                                                                                                                                                                                                                                                                                          |     |
| 模型健康巡检 | **功能概述：**定时（每 5min）对所有已注册模型发送心跳探测请求<br><br>**巡检看板字段：**<br>1. 模型名称<br>2. 最近探测时间<br>3. 响应延迟<br>4. 状态（正常 / 超时 / 不可达）<br><br>**自动化规则：**<br>1. 连续 3 次失败自动将模型状态置为「不可用」并触发告警通知<br>2. 不可用模型在实验创建时自动灰显不可选                                                                                                                                                                                                                                                                 |     |
...
| 功能                | 详细说明                                                                                                                                                                                                                                               | 交互图 |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| 提示词列表             | 展示所有提示词模板；字段：模板名称、用途类型（Agent System Prompt / 评估器 Judge Prompt / 工具指令 Prompt / 通用模板）、关联模型、当前版本号、状态（草稿 / 已发布）、创建人、更新时间；支持按用途类型、状态、关联模型筛选，支持名称和内容关键词搜索；每行支持：查看详情、快速复制、发起测试、归档                                                                         |     |
| 创建提示词模板           | Prompt 编辑器：System Prompt 编辑区（Markdown 语法高亮 + 实时预览）+ 可选 User Prompt 编辑区；支持变量占位符 `{{input}}`、`{{output}}`、`{{expected}}`、`{{context}}`、自定义变量；变量面板列出所有使用变量及其类型说明；分段编辑模式：将长 Prompt 拆分为「角色设定」「任务指令」「输出格式」「约束条件」等段，各段可独立编辑和排序；Token 预估自动计算；关联推荐模型（非强制绑定） |     |
| 版本管理              | 草稿状态可自由编辑，点击「发布版本」后生成版本号（v1.0、v1.1…）并锁定为只读；修改需基于当前版本「创建新版本」进入草稿态；版本列表字段：版本号、发布时间、变更摘要、Token 数变化、关联实验数；Diff 对比：选中两个版本并排展示差异（新增绿 / 删除红 / 修改黄高亮）；实验绑定需指定版本号确保可复现；历史版本只读不可删除                                                                           |     |
| Prompt Playground | 在线调试面板：变量填充（手动输入或从评测集选取样本自动填充）、模型选择（已注册模型）、参数配置（支持直接选择参数预设模板）；执行后展示：模型完整响应（Markdown 渲染）、Token 明细（Input / Output / Total）、延迟（TTFT + 总延迟）、完整请求/响应 JSON（可折叠）；支持多轮对比测试：同一 Prompt 使用不同参数或模型执行多次，结果并排展示                                                  |     |
| 批量测试              | 选择评测集版本作为测试数据源，系统逐条填入 Prompt 变量批量调用模型；返回批量结果概览：成功率、平均延迟、Token 消耗统计；适用于验证 Prompt 修改后在大量 Case 上的整体表现稳定性                                                                                                                                              |     |
| 提示词引用关系           | 展示当前提示词被哪些资源引用：Agent 应用（作为 System Prompt）、评估器（作为 Judge Prompt）、实验配置；修改已发布版本时提示影响范围（如该 Prompt v1.2 被 3 个 Agent 和 5 个评估器引用）；停用前强制确认解除引用关系                                                                                                            |     |
| 内置 Prompt 模板库     | 系统预置常用 Prompt 模板：「通用质量评估」— 评估准确性、完整性、逻辑性；「代码质量评估」— 评估正确性、风格规范、边界处理；「工具调用评估」— 评估工具选择准确性和参数正确性；「安全合规评估」— 检测敏感信息泄露和有害内容；不可修改，可一键「复制为自定义模板」后编辑                                                                                                         |     |
...
| 功能         | 详细说明                                                                                                                                                                                                             | 交互图 |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| Agent 应用列表  | 展示所有接入平台的 Agent 应用；字段：Agent 名称、场景类型（AI Coding / 长上下文 / 视频理解 / 视频生成）、接入方式、运行状态（运行中 / 已停止 / 异常）、创建时间、Agent 描述；支持按场景类型、运行状态多维筛选，支持模糊搜索；每行支持快捷操作：监控详情、发起评估、删除                                                        |  |
| 新建 Agent 应用 | 基础信息：Agent 名称（必填）、场景类型（单选）、描述、负责人；接入配置：接入方式（SDK 接入 / API Endpoint 接入）、AgentKit Runtime 标识；模型绑定：绑定 MaaS 平台已部署模型（可多选），指定默认推理模型；工具配置：声明 Agent 可调用工具列表（工具名称、类型、Endpoint）；环境标签：标记运行芯片型号（A800 / X6000s / NGU800P）和集群环境 |  |
| Agent 应用详情  | 展示 Agent 全部配置信息及当前运行状态；运行历史 Tab：按时间倒序列出所有评估任务记录，支持跳转详情；可观测 Tab：跳转至该 Agent 实时运行监控大盘；版本记录 Tab：展示 Agent 配置变更历史，支持回溯对比不同版本的配置差异                                                                                      |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 评测集列表 | 展示所有评测集；字段：数据集名称、关联 Agent、场景类型、Case 数量、版本号、状态（草稿 / 已发布 / 已归档）、更新时间；支持按 Agent、场景类型、状态筛选；支持搜索数据集名称<br><br>- **评测集**：用于评测评估对象的一组数据，包含输入数据和预期输出结果，帮助开发者验证评估对象的效果。<br>- **输入数据**：提供给评测对象的标准化测试输入。<br>- **预期输出（可选）**：理想输出结果，作为评估基准。 |  |
| 创建评测集 | 基础信息：评测集名称（必填）、描述、关联 Agent（必选）、场景标签；列配置（Schema 定义）：输入列配置（列名、数据类型、查看格式、描述说明，支持文本/文件/多模态）、预期输出列（可选，用于自动评分基准）、自定义扩展列（任务难度等级、场景子类型、标注人等元信息）；数据类型支持 String / JSON / Markdown / Code；评分维度配置：选择适用的评分维度（任务完成率、工具调用准确率、输出质量等） |  |
| 添加 / 导入 Case 数据 | **本地导入：** 支持 CSV / JSON / XLSX 格式（系统提供标准模板下载），导入时自动校验字段匹配，展示字段映射确认步骤；**手动添加：** 单条录入 Case，按列配置逐字段填写，支持 Markdown 富文本输入；**从历史 Trace 生成（推荐）：** 从已运行的 Trace 链路一键提取 Input/Output，生成 Case 草稿，人工补充预期输出后加入数据集；**Case 预览：** 添加后列表内展开查看每条 Case 完整内容 |  |
| 多模态数据管理 | 支持图片（≤20MB，支持一次多张）、视频（≤100MB，支持一次多个）、音频（≤100MB，支持一次多个）、文件（PDF / Excel）的上传和管理；存储路径为本地路径 URL；多模态数据可与文本数据组合使用 |  |
| 编辑与删除数据 | 仅在**草稿**状态下可修改列配置和测试数据；已发布版本不可修改，需新建版本；支持逐条编辑数据项和删除数据项 |  |
| 版本管理 | 草稿状态可自由编辑，发布后锁定为只读（v1.0、v1.1…）；需修改时基于当前版本「创建新版本」进入草稿态；版本列表：版本号、发布时间、Case 数量变化（+N/-N）、发布备注；版本关联实验任务后不可覆盖；支持版本间 Diff 查看：高亮新增 / 修改 / 删除的 Case 条目；支持一键切换查看历史版本（只读） |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 评估器列表 | 展示所有评估器；字段：评估器名称、类型（规则 / LLM / 人工）、创建人、关联实验数、状态（启用 / 停用）、更新时间；支持按类型、状态筛选，按名称搜索；每行支持快捷操作：详情、复制 |  |
| 创建评估器 — 规则类型 | 评分规则：精确匹配 / 包含关键词 / 正则匹配 / JSON Schema 校验；配置目标字段（对应评测集输出列）；设置评分权重（0～1）；支持多条规则组合（AND / OR 逻辑） |  |
| 创建评估器 — LLM-Judge | 选择 Judge 模型（接入 MaaS 上任意已部署模型）；编写评分 Prompt 模板（System + User），支持变量占位符 `{{input}}`、`{{output}}`、`{{expected}}`；配置评分量表（1-5 分 Likert / Pass/Fail）；支持 Chain-of-Thought 开关（Judge 输出思考过程供人工审查）；可选添加 User Prompt 强调特定评估规则（优先级高于 System Prompt）；支持多模态变量 |  |
| 创建评估器 — 代码类型 | 上传 Python 评分函数，定义输入参数（`input`、`output`、`expected`）和返回格式；支持在线调试 |  |
| 创建评估器 — 人工审核 | 配置评分维度与说明（供评审人参考）；设置评分量表（1-5 / Pass/Fail / 多维打分）；配置评审人列表，支持多人评审取均值 |  |
| 评估器测试 | 在创建/编辑时提供「快速测试」入口；手动输入 Input / Output / Expected，实时预览评分结果；代码评估器额外显示执行日志与运行时间；LLM-Judge 评估器显示 Judge 完整推理过程（含 CoT）；测试通过后方可保存并发布 |  |
| 评估器版本管理 | 每次保存评估器配置自动生成版本记录（v1、v2…）；版本列表：版本号、修改摘要、创建时间、关联实验数；实验绑定评估器时需指定版本号确保可复现；历史版本只读不可删除（保障实验可溯源） |  |
| 预置评估器 | 内置通用评估器：BLEU / ROUGE、JSON 格式校验、工具调用正确率、代码可执行性检测等；不可修改，可直接引用 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 实验列表 | 展示所有实验；字段：实验名称、关联 Agent、绑定评测集、评估器组、运行次数、最近运行时间、最近运行状态（待运行 / 运行中 / 已完成 / 失败）；支持按 Agent、状态、场景类型筛选；每行支持：查看详情、快速运行、复制实验配置、归档 |  |
...
| 运行实验（Run） | 点击「运行」后创建 Run 记录，状态流转：排队中 → 运行中 → 评分中 → 已完成 / 已失败；实时展示进度（已完成 Case / 总数）、当前成功率、ETA；支持中途暂停（保留已完成 Case 结果）和强制中止；运行完成后自动进入评分流程；每条 Run 关联完整 Trace 数据（通过注入 `experiment_id` + `run_id` 至 Span 元数据） |  |
| 单实验结果分析 | **汇总指标看板：** 效果指标（任务完成率、各评分维度均值）+ 性能指标（平均 TTFT、端到端延迟、Token 消耗、总成本）+ 评分分布（饼图/柱状图展示各分数段占比）；**Case 明细列表：** 每条展示 Case ID、输入摘要、实际输出、预期输出、综合评分、各评估器分项得分、耗时、Token 数、状态（通过 / 失败 / 待人工审核）；支持按评分区间、状态、耗时筛选和排序 |  |
| Case 完整详情页 | 展示完整输入/输出（Markdown 渲染）；评分明细：各评估器分项得分 + 评分理由（LLM-Judge 提供 CoT 推理过程）；Trace 链路跳转：一键跳转关联 Trace 详情（通过 `case_id` + `run_id` 定位）；模型参数回显（模型名称、Temperature、提示词版本等）；BadCase 标注：可标记为 BadCase 并填写归因标签和备注 |  |
| 多实验比对分析 | **指标统计对比：** 以实验为列、指标为行横向对比（完成率、各评分均值、TTFT、延迟、Token 消耗、成本等），差异超阈值自动标红（退步）/ 标绿（进步），可设基准实验以差值/百分比展示；**逐 Case 比对视图：** 相同 Case ID 各实验下输出并排展示，文本 Diff 高亮，支持按「评分差异最大」排序；**性能散点图：** X 轴 TTFT/延迟、Y 轴质量评分，颜色区分实验，SLO 参考线四象限标注，支持框选异常区域下钻 |  |
| 跨芯片比对模式 | 选择「芯片对比模式」，系统自动识别实验芯片标签并分组展示；核心比对维度：TTFT 差异、吞吐量差异、成本比（¥/M-token）、任务完成率差异、精度损失（量化前后 Token 一致率）；芯片商用可行性评估：对照 4.1 章节 SLO 阈值逐项自动判定 ✅ 达标 / ⚠️ 接近阈值 / ❌ 未达标；每个维度可展开查看 Case 级散点分布；一键生成芯片对比结论摘要卡片 |  |
| BadCase 标注与管理 | 在比对视图或 Case 详情中标记 BadCase；归因标签：模型问题 / 工具调用异常 / 芯片推理偏差 / 数据问题 / 提示词缺陷 / 其他；BadCase 池汇总所有标记，支持按归因分类筛选和导出；BadCase 可一键回流至评测集作为后续回归测试用例 |  |
| 运行历史 | 同一实验多次 Run 记录（Run ID、运行时间、成功率、耗时）；支持查看每次运行的详细评估报告 |  |
| 克隆实验 | 基于已有实验快速创建新实验（保留配置，允许修改部分参数）；适用于 A/B 测试场景 |  |
| 比对报告导出 | 一键生成结构化比对报告，支持 PDF / Markdown / CSV 格式；报告内容：实验配置对比表、指标对比表、BadCase 清单（含归因统计）、散点图截图、芯片可行性结论；导出时可选择报告章节 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 总览大盘 | 时间筛选器：最近 1h / 6h / 24h / 7d / 自定义；全局指标卡片：Runtime 数、活跃会话数、总调用数、总 Token 消耗、错误数、错误率；趋势图组：调用数 / 错误率 / Token 消耗趋势（均支持按 Agent 下钻）；告警区域：实时展示当前触发的告警规则，支持跳转至对应 Trace |  |
| Runtime 列表 | 展示所有 Runtime 实例；字段：Runtime 名称、关联 Agent、当前状态、会话数、调用数、错误率、平均延迟；点击进入单 Runtime 详情（CPU / 内存 / 线程 / 上下文切换趋势图）；支持按 Agent / 状态 / 芯片环境筛选 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| Trace 列表 | 字段：Trace ID、关联 Agent / 实验 / Run ID、触发时间、端到端延迟、Span 总数、LLM 调用次数、Token 消耗（输入/输出）、状态（成功 / 失败 / 超时）；**多维筛选：** Agent、时间范围、状态、延迟区间（< 1s / 1–5s / > 5s）、关联 Case ID、Run ID、模型名称；**快速定位：** 通过 Case ID 直接定位关联 Trace（评估任务运行时自动注入 `case_id`、`run_id` 至 Span 元数据）；排序：按端到端延迟、Token 消耗、Span 数量排序；内联预览 Top-3 最耗时 Span |  |
| Trace 详情 — 瀑布图 / 火焰图 | **瀑布图视图（默认）：** 甘特图展示完整 Span 树，着色：LLM 调用（蓝）/ 工具调用（橙）/ 记忆库（绿）/ 知识库（青）/ 框架逻辑（灰）；性能瓶颈 Span（耗时 > 总时长 30%）标记 🔥；error / timeout 红色高亮；**火焰图视图（切换）：** 按调用层级聚合，横轴为时间，纵轴为调用栈深度，适合分析深层嵌套 Agent 结构；**接口精简视图：** 仅展示 Server 或 Consumer 角色 Span，适用于复杂调用快速提取关键信息 |  |
| Span 详情侧边栏 | 点击任意 Span 触发侧边栏：**LLM 类型** — 完整 Prompt（System / User / Assistant 分段 + Markdown 渲染）、Response（含 Function Call）、模型参数（模型名 / Temperature / Top-P / Max Tokens）、Token 明细（Input / Output / Cache Hit）、性能（TTFT / TPOT / 总延迟）；**工具调用类型** — 工具名称、入参（JSON 格式化）、返回结果（JSON / 纯文本切换）、延迟、完整错误堆栈；**Agent 框架类型** — 节点类型（Planner / Executor / Reflector）、输入/输出状态、子 Span 汇总统计 |  |
| Trace 对比 | 选中 2 条 Trace 进入双栏对比视图；时间轴对齐（以各自起始时间为基准）；差异高亮（耗时差 > 20% 橙色边框，仅出现一侧的 Span 虚线边框）；顶部指标对比面板：总延迟 / TTFT / Token / 工具调用次数 / LLM 调用次数；**典型场景：** 对比同一 Case 在 A800 vs NGU800P 上的 Trace，定位芯片差异引起的延迟分化 |  |
| 性能分析 | 对筛选范围内 Trace 进行 Span 级聚合；每类 Span（按 `span_name` 聚合）展示 P50 / P90 / P99 耗时分布图；慢 Span 排行 Top-10（调用频次、平均/最大耗时）；调用频次热图（时间 × Span 类型）；筛选条件：至少指定 Agent + 时间范围 |  |
| 错误传播分析 | 对含错误 Span 的 Trace 进行根因分析；**错误溯源图：** 有向图展示错误 Span 上游调用链，判断错误由哪层触发；错误分类统计（超时 / 模型报错 / 工具异常 / 网络问题），频率与影响 Trace 数；错误时间线：识别集中爆发时段；一键创建 BadCase 并添加归因标签 |  |
| 实验级 Trace 聚合视图 | 「实验视图」Tab 按实验维度聚合展示 Trace：**一级视图** — 各实验 Trace 汇总（实验名、Run 次数、Trace 总数、平均延迟、错误率、Token 总量）→ **二级视图** — 按 Run ID 分组（Run ID、运行时间、Case 总数、Trace 数、平均延迟）→ **三级视图** — 按 Case ID 列出每条 Case 关联 Trace 及关键指标；任意层级可点击进入 Trace 详情 |  |
| Case ↔ Trace 双向导航 | **Case → Trace：** 从实验结果 Case 明细列表点击「查看 Trace」，通过 `case_id` + `run_id` 自动匹配跳转；多条 Trace 时按时间顺序列出；**Trace → Case：** Trace 详情页顶部关联信息卡片（关联实验名、Case ID、评测集版本、运行环境标签），支持反向跳转；面包屑导航：`实验名 > Run ID > Case ID > Trace ID` 逐级回溯 |  |
| 评测集数据回流 | 选中一条或多条 Trace 点击「回流至评测集」，系统自动提取 Input（首轮 User Prompt）和 Output（最终 Agent 响应），生成 Case 草稿（填充 `input`、`actual_output`、`source: from_trace`）；用户补充 `expected_output` 和 `scenario_tag` 后加入指定评测集草稿版本；**典型场景：** 从日常 Trace 中发现有价值异常 Case，直接沉淀为回归测试用例 |  |
| 实验 Trace 批量分析 | 选中实验某次 Run 进入批量分析：**延迟分布热力图** — X 轴 Case 序号、Y 轴 Span 类型、颜色深浅表示耗时；**Span 类型耗时箱线图** — P50/P90/P99 耗时分布；**异常 Trace 聚类** — 自动识别偏离均值的 Trace 按模式分组；**LLM 调用模式分析** — 调用次数分布、平均 Token 消耗、Cache Hit 率 |  |
| 跨芯片 Trace 批量对比 | 选中同一评测集在不同芯片两次 Run：按 Case ID 对齐 Trace 逐 Case 展示延迟差异；**差异分布图** — X 轴 Case 序号、Y 轴延迟差异（%）；瓶颈定位：自动标注延迟差异 Top-10 Case 及差异最大 Span 类型；**典型场景：** 批量对比 A800 vs NGU800P 系统性定位芯片差异 |  |
| Span 导出 | 将筛选后 Span / Trace 数据导出为 CSV / JSON；包含 Trace ID、Span 耗时、Token 消耗、LLM 参数等关键指标字段 |  |
| 日志分析 | **Runtime 运行日志：** 分词检索 + 按 Runtime 名称 / 日志级别 / Trace ID 筛选，用于故障排查和行为追踪；**Runtime 构建日志：** 持续交付流水线构建过程日志，用于构建故障排查 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 会话列表 | 按会话 ID 聚合多轮交互；字段：会话 ID、关联 Agent / Runtime、开始时间、总耗时、Total Tokens、Input Tokens、Output Tokens、交互轮次（调用链数）、工具调用次数、用户 ID、状态；**多维筛选：** Runtime 名称、Agent、会话 ID、用户 ID、耗时区间、状态；**排序：** 按总耗时 / Token 消耗 / 交互轮次排序，快速找到异常长会话 |  |
| 会话详情 | 展示完整多轮对话历史，每轮显示 Input / Output（Markdown 渲染）、工具调用记录、Trace 跳转链接；**会话级汇总：** 总 Token（Input / Output）、总耗时、平均轮次延迟、工具调用成功率；支持输入/输出格式切换（原始 JSON / Markdown / 纯文本） |  |
| 调用链详情（Trace 跳转） | 每轮次右侧「查看 Trace」按钮，直达该轮 LLM 调用完整链路；可查看 LLM 数据（Input / Output、Request 系统 / 模型 / 类型）、Response（模型、缓存命中 Token、Input / Output / 总 Token 消耗）、LLM 指标看板 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 模型监控 | **LLM 性能看板：** 模型调用总次数、模型调用平均耗时、模型调用错误次数（指标卡）；模型调用次数 / 平均耗时 / 错误次数趋势图；模型调用次数 / 平均耗时 / 错误次数排行 TOP5；模型请求耗时分位数 P99 / P90 / P75 / P50；TTFT 分位数 P99 / P90 / P75 / P50；TPOT 分位数（首 Token 后每个 Token 生成时间） |  |
| Token 分析 | Token 使用总量（全量 + 分模型统计）；LLM 单次调用消耗 Token 及趋势；Token 使用模型排行 Top5；Token 使用总量趋势 |  |
| 推理算力监控 | 跳转至运行时监控页签，查看推理引擎运行时监控：CPU 耗时、CPU 使用率、内存使用、GC 对象数量、线程数量、上下文切换数量等 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 效果指标看板 | 任务完成率趋势（按实验 Run 维度折线图，多实验叠加对比）；评分维度雷达图（任务完成率、决策准确率、工具调用正确率、输出质量 ROUGE/BERTScore、安全合规率）；评分分布直方图（Case 评分分段柱状图）；BadCase 归因统计饼图（支持下钻查看 Case 列表） |  |
| 性能指标看板 | TTFT 分位数对比（柱状图 + 误差线，超 SLO 标红）；端到端延迟分布箱线图（Min / Q1 / Median / Q3 / Max / 异常值）；吞吐量对比柱状图（token/s，分组并排）；Token 消耗瀑布图（System Prompt + User Input + Agent 推理 + Tool 调用 + Output）；TTFT vs 输出质量散点图（SLO 参考线四象限标注）；延迟趋势折线图（TTFT / TPOT / E2E，按分钟/小时粒度） |  |
| 算力比对看板 | **芯片性能雷达图：** 6 维归一化评分（TTFT、吞吐量、MFU 利用率、成本效率 token/¥、稳定性 P99/P50 比值、任务完成率）；**芯片指标对比表：** 行为芯片型号、列为核心指标，差异 > 10% 自动标色；**延迟对比瀑布图：** Prefill Time + Decode Time + 工具调用等待 + 网络传输分段对比；**吞吐量-功耗散点图：** 每瓦吞吐效率等值线；**成本效率对比：** ¥/req、¥/M-token、¥/h + 成本构成叠加；**SLO 达标率面板：** ✅ 达标 / ⚠️ 接近 / ❌ 未达标矩阵 + 综合结论；**硬件利用率对比：** GPU / 显存 / MFU / MBU / CPU / 内存利用率；**跨集群扩展性曲线：** 随节点数增长的吞吐量/延迟/成本趋势 |  |
| 成本指标看板 | 单请求成本趋势（¥/req 折线图）；Token 单价对比（Input / Output ¥/M-token 分组柱状图）；成本构成桑基图（算力资源 → 推理阶段 → 场景应用）；ROI 评估看板（节点 ROI、投入产出比、单位产能成本 + 行业基准对比） |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 报告列表 | **功能概述：**展示所有已生成的报告，支持筛选与管理操作<br><br>**列表字段：**<br>1. 报告名称<br>2. 关联实验/Run<br>3. 生成时间<br>4. 报告类型（单实验报告 / 比对报告 / 芯片商用报告 / 周期汇总报告）<br>5. 导出状态（生成中 / 已完成 / 失败）<br>6. 文件格式<br><br>**筛选与操作：**<br>1. 支持按类型、时间、关联 Agent 筛选<br>2. 每行支持：预览、下载、重新生成、删除 |  |
| 单实验验证报告 | **功能概述：**实验完成后系统自动生成（也可手动触发）<br><br>**报告结构：**<br>1. 封面与基础信息（验证时间、Agent、数据集版本、芯片/集群、模型版本、提示词版本）<br>2. 效果指标汇总（指标卡 + 雷达图）<br>3. 性能指标汇总（TTFT 分位数柱状图、延迟箱线图、吞吐量柱状图、Token 瀑布图）<br>4. 成本指标汇总（Token 消耗、¥/M-token、¥/req、¥/h + 饼图）<br>5. BadCase 清单（归因统计饼图 + Trace 链接）<br>6. 关键洞察与建议（自动生成 3～5 条优化建议）<br><br>**导出格式：**<br>1. PDF<br>2. Markdown<br>3. HTML |  |
| 芯片商用可行性报告 | **功能概述：**基于跨芯片比对数据自动生成商用可行性评估报告<br><br>**报告结构：**<br>1. 芯片性能雷达图<br>2. SLO 达标矩阵（逐指标判定）<br>3. 延迟对比图组<br>4. 成本效率对比<br>5. 算力利用率对比<br>6. 综合结论（Pass / Conditional Pass / Fail）<br><br>**审批流程：**<br>1. 可配置审批流 |  |
| 比对报告 | **功能概述：**基于多实验比对结果自动生成对比分析报告<br><br>**报告内容：**<br>1. 指标对比表<br>2. 逐 Case 差异摘要<br>3. BadCase 对比统计<br>4. 可视化图表截图：<br>a. 评分雷达对比<br>b. 延迟箱线对比<br>c. 性能散点图<br>5. 每项指标附变化率标注（较基准 +/-XX%） |  |
| 周期汇总报告 | **功能概述：**按周 / 月 / 里程碑自动汇总所有验证活动<br><br>**报告内容：**<br>1. 验证场景覆盖率<br>2. 实验次数统计<br>3. 核心指标趋势折线图<br>4. BadCase 趋势<br><br>**适用场景：**<br>1. 向管理层汇报验证进度和芯片优化效果 |  |
| 报告版本对比 | **功能概述：**同一 Agent/场景多份历史报告纵向对比<br><br>**核心能力：**<br>1. 时间轴视图展示关键指标历史变化趋势<br>2. 支持标注关键版本节点（首次验证、优化后验证、商用评审等里程碑）<br>3. 环比变化 > 15% 自动标注 |  |
| 报告模板与自定义 | **功能概述：**提供内置报告模板与自定义能力<br><br>**内置模板：**<br>1. 标准验证报告<br>2. 芯片商用报告<br>3. KA 客户演示报告<br><br>**自定义能力：**<br>1. 支持自定义报告章节和图表组合<br>2. 支持添加自定义文字段落<br>3. 自定义 Logo 和页眉页脚（适用于对外输出） |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 平台概览 KPI 卡片 | **功能概述：**顶部指标卡片组，展示平台核心运行数据<br><br>**卡片指标：**<br>1. 运行中实验数<br>2. Runtime 总数<br>3. Total Tokens 消耗量<br>4. 异常告警数<br><br>**交互能力：**<br>1. 支持时间范围切换（近 24h / 7d / 30d）<br>2. 默认展示近 24h 数据 |  |
| 调用趋势图 | **功能概述：**近 24h LLM 调用次数和工具调用次数趋势折线图<br><br>**图表配置：**<br>1. 支持按 Agent 维度下钻<br>2. 横轴按小时粒度<br>3. 纵轴为调用次数 |  |
| 告警概览 | **功能概述：**展示实时告警列表<br><br>**列表字段：**<br>1. 告警类型（模型连接超时 / KV Cache 命中率骤降 / Runtime 内存告警）<br>2. 级别（严重 / 警告 / 信息）<br>3. 触发时间<br>4. 关联模块<br><br>**操作：**<br>1. 支持一键跳转告警来源 |  |
| Token 消耗排行 | **功能概述：**展示 Top 5 模型 Token 消耗柱状图<br><br>**交互能力：**<br>1. 支持切换 Input / Output / Total Token 维度 |  |
| 最近实验 | **功能概述：**展示最近实验活动列表<br><br>**列表字段：**<br>1. 实验名称<br>2. 关联 Agent<br>3. 运行状态<br>4. 进度条<br>5. 最近运行时间<br><br>**操作：**<br>1. 点击跳转实验详情 |  |
| 导出报告快捷入口 | **功能概述：**右上角「导出报告」按钮，快速导出仪表盘数据摘要<br><br>**导出格式：**<br>1. PDF<br>2. Markdown |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 用户列表 | **功能概述：**展示所有平台用户，支持筛选与搜索<br><br>**列表字段：**<br>1. ID<br>2. 账号（邮箱）<br>3. 用户名<br>4. 角色（7 种预置角色）<br>5. 部门<br>6. 创建人<br>7. 创建时间<br>8. 最近登录<br>9. 状态（启用 / 禁用）<br><br>**筛选与操作：**<br>1. 支持按角色 / 部门 / 状态筛选<br>2. 支持模糊搜索 |  |
| 新增用户 | **功能概述：**通过表单新增平台用户<br><br>**表单字段：**<br>1. 邮箱（必填，唯一性校验）<br>2. 用户名<br>3. 角色<br>4. 部门<br>5. 初始密码（自动生成或手动）<br><br>**提交行为：**<br>1. 提交后邮件通知 |  |
| 编辑用户信息 | **功能概述：**编辑已有用户的信息<br><br>**可编辑项：**<br>1. 修改基本信息<br>2. 角色重分配<br>3. 部门变更<br><br>**交互提示：**<br>1. 角色变更弹出影响范围提示 |  |
| 批量操作 | **功能概述：**对多个用户执行批量管理操作<br><br>**批量能力：**<br>1. 批量重置密码<br>2. 批量启用 / 禁用<br><br>**安全机制：**<br>1. 操作前确认对话框 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 角色列表 | **功能概述：**展示所有预置与自定义角色<br><br>**预置角色（7 种）：**<br>1. 超级管理员<br>2. 租户管理员<br>3. 验证工程师<br>4. 测试工程师<br>5. BadCase 评审员<br>6. 只读用户<br>7. 外部客户<br><br>**列表字段：**<br>1. 角色名<br>2. 描述<br>3. 当前用户数<br>4. 创建时间<br>5. 类型（预置 / 自定义）<br><br>**约束规则：**<br>1. 预置角色不可删除 |  |
| 角色权限矩阵 | **功能概述：**按模块分配权限，以勾选框矩阵展示<br><br>**权限模块：**<br>1. 概览仪表盘<br>2. 资源管理（模型 / 提示词 / Agent）<br>3. 评测管理（评测集 / 评估器 / 实验）<br>4. 可观测（Trace / 会话）<br>5. 报告管理<br>6. 系统设置（用户 / 角色 / API / 标签 / 审计）<br><br>**交互形式：**<br>1. 权限矩阵勾选框展示 |  |
| 自定义角色创建 | **功能概述：**创建自定义角色并分配权限<br><br>**创建表单：**<br>1. 输入角色名<br>2. 输入描述<br>3. 逐项勾选功能权限<br><br>**管理操作：**<br>1. 可编辑和删除（需先解绑关联用户） |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| MaaS 连接列表 | **功能概述：**展示所有已配置的 MaaS 连接<br><br>**列表字段：**<br>1. 名称<br>2. 类型（内部 / 外部 / 自部署）<br>3. Endpoint<br>4. 连接模型数<br>5. 数据源（Prometheus / vLLM / API / SGLang）<br>6. 最近同步时间<br>7. 健康度<br>8. 状态<br><br>**视觉规则：**<br>1. 异常红色高亮 |  |
| KPI 卡片 | **功能概述：**展示 API 管理模块核心指标<br><br>**卡片指标：**<br>1. 已连接 MaaS 数<br>2. 连接模型总数<br>3. 聚合调用次数<br>4. 平均 TTFT<br><br>**趋势展示：**<br>1. 各指标较上周期变化趋势 |  |
| 新建 MaaS 连接 | **功能概述：**通过配置表单新建 MaaS 连接<br><br>**表单字段：**<br>1. 连接名称<br>2. 类型<br>3. Endpoint URL<br>4. 认证（API Key / Token）<br>5. 数据源类型<br>6. 同步间隔（5min / 15min / 30min / 1h）<br><br>**提交行为：**<br>1. 提交前自动连通性测试 |  |
| 连接健康监控 | **功能概述：**监控 MaaS 连接的健康状态<br><br>**监控能力：**<br>1. 自动健康检查（定时探测）<br>2. 手动连通性测试<br>3. 延迟监控<br><br>**告警机制：**<br>1. 连续 3 次失败自动标异常并告警 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 标签分类管理 | **功能概述：**管理标签分类维度<br><br>**分类维度：**<br>1. 芯片型号（A800 / NGU800P / X6000s）<br>2. 集群标识<br>3. 场景类型<br>4. Agent 类型<br>5. 难度等级等<br><br>**操作：**<br>1. 支持增删改分类 |  |
| 标签 CRUD | **功能概述：**标签的增删改查管理<br><br>**基础操作：**<br>1. 创建标签<br>2. 编辑标签<br>3. 删除标签（删除前提示关联资源数量）<br><br>**高级操作：**<br>1. 批量创建 / 删除<br>2. 标签合并（同义标签合并，自动迁移关联） |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 操作日志列表 | **功能概述：**记录所有用户操作和系统变更<br><br>**列表字段：**<br>1. 时间<br>2. 用户<br>3. 模块<br>4. 操作类型（创建 / 更新 / 删除 / 导出 / 登录）<br>5. 操作对象<br>6. 来源 IP<br><br>**筛选与操作：**<br>1. 支持多条件筛选<br>2. 支持全文搜索 |  |
| 日志导出 | **功能概述：**将筛选后的日志数据导出<br><br>**导出能力：**<br>1. 导出为 CSV 格式<br>2. 支持选择导出时间范围 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 通知列表 | **功能概述：**展示所有通知消息<br><br>**通知类型：**<br>1. SLO 告警<br>2. 模型健康告警<br>3. 实验完成<br>4. 报告生成<br>5. 系统通知<br><br>**列表字段：**<br>1. 时间<br>2. 类型<br>3. 标题<br>4. 内容<br>5. 状态（未读 / 已读）<br><br>**视觉规则：**<br>1. 未读加粗 + 角标 |  |
| 通知渠道配置 | **功能概述：**配置通知推送渠道<br><br>**推送渠道：**<br>1. 站内通知（默认）<br>2. 邮件<br>3. Webhook<br>4. 飞书<br>5. 钉钉<br><br>**配置能力：**<br>1. 每渠道独立配置<br>2. 测试通知 |  |
| 告警规则管理 | **功能概述：**管理告警触发规则<br><br>**规则配置项：**<br>1. 监控指标<br>2. 阈值<br>3. 条件<br>4. 级别（严重 / 警告 / 信息）<br>5. 通知渠道（多选）<br><br>**管理操作：**<br>1. 规则列表支持启用/禁用 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 集群列表（只读） | **功能概述：**通过 MaaS API 同步获取所有可用集群信息<br><br>**列表字段：**<br>1. 集群名称<br>2. 集群 ID<br>3. 芯片型号（A800 / NGU800P / X6000s）<br>4. 节点数<br>5. GPU 总卡数<br>6. 可用卡数<br>7. 集群状态（在线 / 离线 / 维护中）<br>8. MaaS 来源<br>9. 最近同步时间<br><br>**筛选与同步：**<br>1. 支持按芯片型号和状态筛选<br>2. 定时自动同步（同步间隔与 API 管理模块一致） |  |
| 集群详情 | **功能概述：**展示单个集群的详细信息，所有数据只读，通过 MaaS API 实时拉取<br><br>**详情内容：**<br>1. 硬件配置（芯片型号、显存规格、互联拓扑）<br>2. 已部署模型列表<br>3. 当前负载概览：<br>a. GPU 利用率<br>b. 显存使用率<br>c. 网络带宽<br>4. 关联实验列表（在该集群上运行过的实验记录） |  |
| 集群环境标签 | **功能概述：**为集群分配 AaaS 平台内的标签<br><br>**标签类型：**<br>1. 芯片型号<br>2. 集群代号<br>3. 部署环境等<br><br>**用途：**<br>1. 实验创建时选择目标环境<br>2. 跨集群比对时的分组依据<br><br>**联动：**<br>1. 标签与「标签管理」模块联动 |  |
| 跨集群对比视图 | **功能概述：**选择 2～4 个集群，横向对比核心指标<br><br>**对比指标：**<br>1. GPU 利用率<br>2. 显存使用率<br>3. 平均 TTFT<br>4. 吞吐量（tokens/s）<br>5. 成本效率（¥/M-token）<br><br>**筛选与规则：**<br>1. 支持按时间范围筛选<br>2. 差异超过 15% 自动高亮 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 资产总览与文件管理 | **功能概述：**统一管理平台数据资产，支持多格式上传与检索<br><br>**KPI 卡片：**<br>1. 资产总数<br>2. 存储占用<br>3. 引用数<br><br>**支持格式：**<br>1. PDF / DOCX / XLSX<br>2. Markdown<br>3. 代码仓库<br>4. JSON / CSV<br>5. 图片<br>6. 音视频<br><br>**核心能力：**<br>1. 自动计算 SHA256 哈希<br>2. 目录树组织 + 全文检索 |  |
| Agent 知识库 | **功能概述：**面向 Agent 场景的知识库管理<br><br>**分组维度：**<br>1. 产品文档<br>2. 代码仓库<br>3. FAQ 语料<br>4. 多轮对话样本<br><br>**索引能力：**<br>1. 支持文档索引（全文索引 + 向量索引）<br>2. 索引状态追踪（已索引 / 索引中 / 待索引）<br><br>**筛选与操作：**<br>1. 支持搜索和分组筛选 |  |
| 多模态素材库 | **功能概述：**管理多模态素材资源<br><br>**支持素材类型：**<br>1. 图像（PNG/JPG/HEIC/SVG）<br>2. 音频（MP3/WAV/FLAC）<br>3. 视频（MP4/MOV/WebM）<br>4. PDF<br><br>**展示字段：**<br>1. 素材格式<br>2. 引用次数<br>3. 大小<br><br>**筛选与操作：**<br>1. 支持按素材类型筛选<br>2. 支持关键词搜索 |  |
| 资产版本管理 | **功能概述：**管理资产的版本历史<br><br>**版本规则：**<br>1. 同一资产上传新版本自动递增版本号<br>2. 历史版本不可删除保证追溯<br>3. 评测集 Case 引用时锁定到具体版本号 |  |
| 资产引用与沙箱挂载 | **功能概述：**管理资产与 Case/实验的引用关系<br><br>**核心能力：**<br>1. Case 可从资产库选择文件作为 attachments<br>2. 引用追踪（反向关联列表）<br>3. 实验运行时关联资产自动挂载到 Agent 沙箱 |  |
| 代码仓库快照 | **功能概述：**Coding Agent 场景专用，管理代码仓库快照<br><br>**核心能力：**<br>1. 上传代码仓库快照（git archive / zip）<br>2. 绑定 commit hash<br>3. 绑定 Docker 镜像<br>4. 绑定 F2P/P2P 测试集<br>5. 跨芯片一致性保证 |  |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 负载包列表 | 顶部 KPI 卡片：负载包总数、总存储、已交付数、复现成功率；列表字段：名称、关联实验、包内容（Docker + Case + Traj + 结果）、大小、SHA256、状态（已交付 / 已验证 / 打包中 / Parity 未通过）、创建时间、到期时间；支持筛选和批量操作 |  |
| 创建向导 | 四步创建流程：① 选择实验 → ② 打包内容（四层可还原性：L1 环境层 Docker 镜像 bit-exact / L2 输入层评测集+配置 bit-exact / L3 轨迹层 Trajectory+Trace 完整记录 / L4 结果层 Grader+报告 统计可复现）→ ③ 元信息与校验 → ④ 确认打包；附加选项：包含 Mock 数据、生成 MANIFEST.sha256、数据脱敏、压缩存储；右侧实时预估包大小 |  |
| 导入与校验 | SHA256 校验清单逐文件验证（预期 vs 实际）；一键复现配置（复现实验名称、目标芯片、并发数、Seed）；可复现性判定公式：原始 Resolved Rate 95% CI ∩ Pack Resolved Rate 95% CI ≠ ∅ → ✅ 可复现；环境还原流程（解压 Docker → 推到 registry → pip freeze 校验 → 系统包校验 → 复现分析挂载） |  |
| 跨集群交付 | 交付工作流：创建 → 内部验证 → 标记已验证 → 导出交付；KPI：活跃交付数、已验证数、待反馈数、复现成功率；交付追踪：接收方、交付时间、复现结果、反馈状态；自动生成接收方校验清单（README.md）；近期反馈展示（客户复现结果 + 评论） |  |
...
| **性能基准测试**      | **简介**                                             | **能力类型** | 核心测试能力                                                            |  |
| ------------------- | -------------------------------------------------- | -------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| EvalScope模型推理性能压测框架 | 一套“可比、可复现、可扩展”的压测方案                                | 推理压测     | 支持多并发场景下的吞吐量、延迟、稳定性等核心指标测试。<br><br>可模拟真实业务负载，验证模型服务在不同上下文场景下的性能表现 |  |
| MindIE Benchmark    | 昇腾的专门评测框架-能够测量并记录模型在各个阶段的推理耗时（HW 内部生态闭源框架，仅用于参考比对） | 推理压测     | 支持端到端的推理测试                                                        |  |
...
| <font color="#00b050">**评测集名称** | **数据集简介**                </font>                                                                              | **能力类型**          | 核心测试能力                                                                           |  |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------- | ----------------- | -------------------------------------------------------------------------------- | ------ |
...
| 机制 | 生命周期 |  |
|------|----------|----------|
| Subagent（子代理） | 创建 → 执行 → 返回 → 销毁 |  |
| Runtime Task（运行时任务） | 启动 → 运行 → 完成 |  |
| Team Member（团队成员） | 持久存在 → 循环工作 |  |
...
| 错误类型 | 恢复策略 |  |
|----------|----------|------|
| 输出截断 `max_tokens` | 注入续写提示「从中断处继续」 |  |
| 上下文溢出 `prompt too long` | 自动压缩 → 生成摘要 → 从摘要重试 |  |
| 网络/服务错误 `timeout/rate limit` | 指数退避重试 `delay = min(base × 2^attempt, max) + jitter` |  |

**上下文压缩 3 层策略**

| 层级 | 触发条件 | 操作 |  |
|------|----------|------|----------|
| 第一层：大输出持久化 | 工具输出 > 30,000 字符 | 完整内容写磁盘，仅保留 2,000 字符预览 |  |
| 第二层：微压缩 | 常态执行 | 仅保留最近 3 个工具结果完整，更早的替换为占位符 |  |
| 第三层：摘要压缩 | 总上下文 > 50,000 字符 | 完整记录归档，生成摘要保留：目标 / 决策 / 修改文件 / 约束 |  |
...
|------|------|----------|------|
| 理解 | 1 | `read(auth.ts)` |  |
| 分析 | 2 | `grep("import.*auth", "src/")` |  |
| 拆分 | 3-5 | `write(auth/types.ts)` `write(auth/middleware.ts)` `edit(→ auth/index.ts)` |  |
| 修复 | 6-12 | `edit(src/**/*.ts)` × N |  |
| 验证 | 13 | `bash("npm run test")` |  |
| 输出 | 14 | 返回文本 |  |
...
| 阶段 | 执行者 | 操作 |  |
|------|--------|------|------|
| 发现 | 主 Agent | `bash("npm test")` |  |
| 派发 | 主 Agent | `task(子任务A/B/C)` |  |
| 修复 | 子 Agent A | 独立 messages[] → 修复 `auth.test.ts` |  |
| 修复 | 子 Agent B | 独立 messages[] → 修复 `api.test.ts` |  |
| 修复 | 子 Agent C | 独立 messages[] → 修复 `db.test.ts` |  |
| 汇总 | 主 Agent | 收到 3 个文字摘要 |  |
| 验证 | 主 Agent | `bash("npm test")` → 全部通过 |  |

**场景三：团队协作与 Worktree 隔离**

| 阶段 | 角色 | 操作 |  |
|------|------|------|------|
| 规划 | Lead Agent | 创建 3 个 TaskRecord |  |
| 执行 | Teammate A | 在 `worktree/user-api/` 独立开发 |  |
| 解锁 | 系统 | Task1 完成 → 自动解锁 Task2, Task3 |  |
| 并行 | Teammate B/C | 各自 worktree 同时执行 |  |
| 通知 | 全体 | 通过 JSONL 邮箱发送完成通知 |  |
| 合并 | Lead Agent | 审查并合并各 worktree 成果 |  |
...
| 指标类型 | 关注点 | 验证名称 |  |
| ---- | ----- | ----------------------- | ---------- |
| 体验指标 | 用户体验 | 单一任务端到端测试（4.1） |  |
| | 系统吞吐 | 并发压力测试（4.2） |  |
| 诊断指标 | 链路归因 | 过程链路测试指标（4.3）——偏向业务逻辑拆解 |  |
| | 底层定位 | Agent 任务运行时指标（4.4）——偏向底层资源拆解 |  |
| 稳定指标 | 稳定性抖动 | 百分位性能指标测试（4.5） |  |
| 成本指标 | 商业成本 | 单任务 Token 消耗成本测试（4.6） |  |
...
| **重复执行次数** | 10 次 / Case |  |
| **单 Case 模型调用总数** | 80 次 |  |
| **Token 规模** | Input ~81,500 / Output ~5,250 |  |
| **上下文膨胀率** | 4.1× |  |
...
| --- | --- | --- |
| **观测对象** | 单次模型 API 调用 |  |
| **数据粒度** | 请求级：一次请求的 TTFT、TPOT、token 数 |  |
| **覆盖范围** | 仅模型推理链路（Prefill → Decode → 返回） |  |
| **成本视角** | 单次请求成本（¥/req） |  |
| **比较基准** | 同模型不同参数 |  |
...
| 指标 | MaaS 输出 | AaaS 输出 |  |
| --- | --- | --- | --- |
| **TTFT** | 单次请求首 token 时间 | 首轮 TTFT + 末轮 TTFT + 散点图 |  |
| **TPOT** | 单次请求每 token 延迟 | 按 output_tokens 加权的任务级 TPOT |  |
| **ITL** | 单次请求 token 间时延均值 | max(各轮 P99_ITL) |  |
| **E2E Latency** | 单次请求端到端延迟 | 任务级 E2E（含工具+编排）+ 模型推理累计 |  |
| **Input tokens** | 单次请求输入 token 数 | 累计调用量 + 去重信息量 + 上下文膨胀率 |  |
...
| P-02 | 提示词版本管理 | P0 |  |
| P-03 | 变量占位符系统 | P0 |  |
| P-04 | Prompt Playground 调试 | P1 |  |
| P-05 | 多轮对比测试 | P1 |  |
| P-06 | 批量测试（关联评测集） | P2 |  |
| P-07 | 内置 Prompt 模板库 | P1 |  |
| P-08 | 提示词引用关系追踪 | P2 |  |
| P-09 | 分段编辑模式 | P2 |  |
| P-10 | 提示词搜索（内容级） | P1 |  |
...
| 添加数据项 | 在**添加数据**页面，输入第一组测试数据，然后单击 **+ 添加数据项**添加更多测试数据。最后，单击**添加**完成数据添加 |  |
| ----- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| 编辑数据项 | - 修改数据项：在评测集详情页面，单击目标数据项操作列下的**编辑**，然后修改数据并保存                    |  |
| 删除数据项 | - 单条删除：在评测集详情页面，单击目标数据项操作列下的**删除**，即可删除该条数据。                     |  |
...
| 评估器测试   | **功能说明：**<br>1. 在评估器配置页提供「快速测试」入口<br>2. 手动输入 Input / Output / Expected，实时预览评估器打分结果<br>3. 代码评估器额外显示执行日志与运行时间<br>4. LLM-Judge 评估器显示 Judge 的完整推理过程（含 CoT）<br>5. 测试通过后方可保存并发布 |  |
| 评估器版本管理 | **功能说明：**<br>1. 每次保存评估器配置自动生成版本记录（v1、v2…）<br>2. 版本列表显示：版本号、修改摘要、创建时间、关联实验数<br>3. 实验绑定评估器时需指定版本号，确保实验结果可复现<br>4. 历史版本只读，不可删除（保障实验可溯源）                                      |  |
| 调试      | 在完成评估器配置后，单击**调试**，测试一下评估器效果。<br>在弹出的**预览与调试**页面，输入一组测试数据，然后单击**运行**查看评估效果是否符合预期。<br>                                                                                     |  |
...
| 第一步：基础信息 | 填写基础信息。输入实验名称和描述，然后单击 **下一步: 评测集**                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |  |
| 第二步：评测集  | 在 **评测集** 页面，选择你在步骤二创建的评测集，并选择要使用的评测集版本，然后单击**下一步：评测对象**。                                                                                                                                                                                                                                                                                                                                                                                                                                                     |  |
| 第三步：评测对象 | 在 **评测对象** 页面，按照下面的参数说明配置评测对象。然后单击**下一步：评估器**<br><br>                                                                                                                                                                                                                                                                                                                                                                                                                                                         |  |
| 第四步：评估器  | 在 **评估器** 页面，单击 **+添加评估器** 为评测实验设置评估器。评估器配置完成后，单击**确认实验配置**。                                                                                                                                                                                                                                                                                                                                                                                                                                                  |  |
| 最后一步确认页面 | 在页面底部，设置**最大并发执行条数**。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |  |
| 查看实验进度   | 检查实验配置，确认无误后，单击**发起实验**。<br><br>发起实验后，你可以刷新实验页面，查看评估进度。                                                                                                                                                                                                                                                                                                                                                                                                                                                       |  |
...
| 方法二：在实验详情页发起       | 1. 在**实验**列表页面，选择一个目标实验。<br>            |  |
|                    | 2. 在实验详情页，单击**实验对比**。                   |  |
| 方法三：在评测集详情页发起      | 1. 在**评测集**列表页面，选择一个目标评测集。              |  |
|                    | 2. 在**评测集**详情页，单击**关联实验**，然后单击**批量选择。** |  |
|                    | 3. 选择要对比的实验，然后单击**实验对比**。               |  |
| 方法四：在多实验对比看板继续添加实验 | 1. 在实验对比页面，单击 **+ 添加对比实验**。             |  |
...
| | 三、指标体系架构说明 | 三层指标定位（体验→诊断→成本）+ AaaS vs MaaS 口径差异 |  |
| **第二部分：指标明细** | 五、体验指标 | 5.1 单任务基线（含 Agent 多轮调用明细 + 散点图 + 瀑布图）<br>5.2 并发压力测试（10/50/100 Agent 并发梯度 + 高并发质量指标） |  |
| | 5b、稳定指标 | 延迟与吞吐百分位分布（P50/P75/P90/P99/max + SLO 判定）<br>输入输出特征分布<br>推理管线分阶段耗时分布 |  |
| | 六、诊断指标 | 6.1 推理调度指标（单请求延迟拆解：排队/Prefill/Decode/后处理 + ITL Jitter）<br>6.2 Agent 任务质量（跨量化模式对比 FP8/FP16/BF16）<br>6.3 推理引擎调度状态（KV Cache/抢占/Prefix Cache/引擎级统计）<br>6.4 基础设施与集群诊断（存储 IOPS/网络/集群健康） |  |
| | 七、成本指标 | 7.1 硬件利用率（MFU/MBU/GPU/显存/CPU/互联/PCIe/Goodput）<br>7.2 功耗能效（每瓦吞吐 tok/s/W、推理每 token 能耗 J/token）<br>7.3 成本核算（¥/h、¥/req、¥/M-token、节点 ROI + 单任务成本拆解） |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 平台概览 KPI 卡片 | **功能说明：**<br>1. 顶部展示核心 KPI 指标卡片组：运行中实验数、Runtime 总数、Total Tokens 消耗量、异常告警数<br>2. 每张卡片展示当前值和较上周期的变化趋势（↑/↓ + 百分比）<br>3. 支持时间范围切换：近 24h / 7d / 30d，默认展示近 24h 数据<br>4. 点击卡片可跳转至对应模块详情页 |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 用户列表 | **功能说明：**<br>1. 展示所有平台用户的列表视图<br>2. **字段：** ID、账号（邮箱）、用户名、角色（超级管理员 / 租户管理员 / 验证工程师 / 测试工程师 / BadCase 评审员 / 只读用户 / 外部客户）、部门、创建人、创建时间、最近登录时间、状态（启用 / 禁用）<br>3. **筛选：** 按角色、部门、状态筛选，支持账号和用户名模糊搜索<br>4. 禁用用户灰色显示<br>5. 每行操作：编辑、重置密码、启用/禁用 |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 角色列表 | **功能说明：**<br>1. 展示 7 种预置角色：超级管理员、租户管理员、验证工程师、测试工程师、BadCase 评审员、只读用户、外部客户<br>2. **字段：** 角色名称、描述、当前用户数、创建时间、角色类型（预置 / 自定义）<br>3. 预置角色不可删除，可查看和修改权限配置<br>4. 自定义角色支持编辑和删除 |  |
| 角色权限矩阵 | **功能说明：**<br>1. 按模块分配权限，模块维度包括：<br>   a. 概览仪表盘：查看仪表盘 / 导出报告<br>   b. 资源管理：查看模型 / 注册编辑模型 / 查看提示词 / 编辑提示词 / 查看 Agent / 注册编辑 Agent<br>   c. 评测管理：查看评测集 / 编辑评测集 / 查看评估器 / 编辑评估器 / 创建实验 / 运行实验 / 查看结果<br>   d. 可观测：查看 Trace / 查看会话 / 导出数据<br>   e. 报告管理：查看报告 / 生成报告 / 导出报告<br>   f. 系统设置：用户管理 / 角色权限 / API 管理 / 标签管理 / 审计日志<br>2. 权限矩阵以勾选框形式展示各角色在各功能模块的权限<br>3. 修改后实时保存，变更记录写入审计日志 |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| MaaS 连接列表 | **功能说明：**<br>1. 展示所有已配置的 MaaS 连接<br>2. **字段：** MaaS 名称、类型（内部 / 外部 / 自部署）、Endpoint、连接模型数、数据源（Prometheus / vLLM / API 统计 / SGLang）、最近同步时间、健康度（健康 / 异常 / 不可达）、状态（已连接 / 已断开）<br>3. 异常连接红色高亮<br>4. 每行操作：详情、测试连接、编辑、断开连接 |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 知识库分组管理 | **功能说明：**<br>1. 按知识类型分组展示：**产品文档**（PDF / Markdown / DOCX）、**代码仓库**（GitHub 归档 / monorepo）、**FAQ 语料**（XLSX / CSV / JSON / Markdown）、**多轮对话样本**（对话样本集 / 标注数据）<br>2. 每条记录展示：知识库分组、文档名、索引状态（已索引 / 索引中 / 待索引）、大小、创建人、操作（编辑 / 删除）<br>3. 支持关键词搜索和分组筛选 |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 素材分类管理 | **功能说明：**<br>1. 按素材类型分组展示：**图像素材**（PNG / JPG / HEIC / SVG）、**音频素材**（MP3 / WAV / FLAC）、**视频素材**（MP4 / MOV / WebM）、**PDF 文档**<br>2. 每条记录展示：素材类型、文件名、格式状态（可引用 / 待审核）、大小 + 引用次数、创建人、操作（编辑 / 删除）<br>3. 支持关键词搜索和类型筛选<br>4. 右上角「+ 上传素材」入口 |  |
...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 负载包列表 | **功能说明：**<br>1. 顶部 KPI 卡片：负载包总数（活跃 / 已归档）、总存储（平均大小/Pack）、已交付数（客户 / 内部）、复现成功率（%）<br>2. **列表字段：** 负载包名称、关联实验、包内容类型（Docker + Case + Traj + 结果）、大小、SHA256（前 8 位缩略）、状态（已交付 / 已验证 / 已完成 / 打包中 / Parity 未通过）、创建时间、到期时间<br>3. **操作：** 下载、导出、详情、归档；Parity 未通过的 Pack 展示「解档」操作<br>4. 支持按名称搜索、状态和类型筛选、批量操作<br>5. 4 个 Tab 页签：负载包列表、创建向导、导入与校验、跨集群交付 |  |
| 创建向导 | **功能说明：**<br>1. **四步向导流程：** ① 选择实验 → ② 打包内容 → ③ 元信息与校验 → ④ 确认与打包<br>2. **打包内容配置（四层可还原性）：**<br>   a. **L1 环境层：** Docker 镜像（tar.gz）+ Dockerfile + pip freeze + 系统包列表（bit-exact）<br>   b. **L2 输入层：** 评测集（N Case）+ 模型权重 hash + Scaffold 版本 + Prompt hash + seed（bit-exact）<br>   c. **L3 轨迹层：** Trajectory JSONL 全量 + Trace 数据导出（完整记录）<br>   d. **L4 结果层：** Grader 评分结果 + Parity 矩阵 + 汇总指标 + 报告 PDF（统计可复现）<br>3. **附加选项：** 包含 Mock 服务数据集 / 生成 MANIFEST.sha256 校验清单 / 包含 README.md 接收方指南 / 数据脱敏（客户交付模式） / 压缩存储（节省 40% 但非 bit-exact）<br>4. **右侧内容汇总面板：** Pack 名称、关联实验、评测集、Scaffold、模型、芯片信息、各层大小预估、预估总大小、预计打包耗时 |  |
| 导入与校验 | **功能说明：**<br>1. **SHA256 校验清单：** MANIFEST.sha256 全部校验通过 / 失败统计；逐文件展示：文件路径、预期 SHA256、实际 SHA256、校验状态（✅ 一致 / ❌ 不一致 / ⚠️ 全量一致）<br>2. **一键复现配置：** 复现实验名称（自动生成）、目标芯片（NGU800P / A800 / 其他）、并发数、Seed（固定）<br>3. **可复现性判定：**<br>   - 判定公式：原始 Resolved Rate 95% CI ∩ Pack Resolved Rate 95% CI ≠ ∅ → ✅ 可复现<br>   - 展示原始负载包基准值和 CI 范围<br>4. **环境还原流程：** ① 解压 Docker 镜像 → ② 推送到 local registry → ③ 验证 pip freeze 一致性 → ④ 验证系统包完整 → ⑤ 复现分析挂载 |  |
| 跨集群交付 | **功能说明：**<br>1. 顶部 KPI：活跃交付数、已验证数、待反馈数、复现成功率<br>2. **交付列表：** 负载包名称、接收方（客户/团队名称）、交付时间、大小、接收方状态（已复现 / 复现中 / 待复现）、复现结果（Resolved Rate 对比）、反馈数<br>3. **交付工作流：** ① 创建负载包 → ② 内部验证（本地复现一次）→ ③ 标记已验证 → ④ 导出交付（生成 MND URL + 签名）→ ⑤ 接收方复现 → ⑥ 反馈收集<br>4. **接收方校验清单：** 自动生成 README.md（解压验证 → 执行 verify.sh → docker load → 导入评测集 → 使用 seed=N 执行复现 → 对比 Resolved Rate）<br>5. **近期反馈：** 展示各接收方的复现结果和评论 |  |
