...
| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| 运行时状态总览 | **功能说明：**<br>1. 顶部状态栏：展示当前运行中的实验数量、总 Runtime 实例数、活跃会话数<br>2. **实时指标卡片组：** 当前 QPS、平均 TTFT、平均端到端延迟、当前错误率、Token 消耗速率（token/s）<br>3. 时间范围：固定展示最近 15min 滚动窗口，支持切换至 1h / 实验全程<br>4. **告警横幅：** 当任意指标超出 SLO 阈值时，顶部自动弹出告警横幅，标注超阈值指标名、当前值与阈值，支持一键跳转至对应 Trace | [📐 设计稿 — 运行时总览页](https://www.figma.com/design/MdIym00fvsMTq4htpqfO2K/AaaS-%E9%AA%8C%E8%AF%81%E5%B9%B3%E5%8F%B0?node-id=379-1428) — 展示 Runtime 卡片、状态和指标 |
...
| 资产引用与关联 | **功能说明：**<br>1. **评测集关联：** 在评测集 Case 编辑时，可从资产库选择文件作为 `attachments`<br>2. **引用追踪：** 资产详情页展示「被哪些评测集 / Case / 实验引用」的反向关联列表<br>3. **引用锁定：** 被实验引用的资产版本自动锁定，防止误删或覆盖<br>4. **沙箱挂载：** 实验运行时，Case 关联的资产文件自动挂载到 Agent 运行沙箱的指定路径<br>5. **Mock 数据关联：** 资产可作为 Mock 服务的响应数据源（联动 5.19 Mock 管理） | — |
...
| 资产导入导出 | **功能说明：**<br>1. **批量导入：** 支持从内网文件服务器批量同步（配置源路径 + 定时同步）<br>2. **资产包导出：** 将选中资产 + 元数据打包为 Reproducibility Pack（ZIP），用于跨集群交付<br>3. **导出内容：** 资产文件 + 元数据 JSON + SHA256 校验清单 + 引用关系映射<br>4. **导入校验：** 导入资产包时自动校验 SHA256，不一致则拒绝并提示差异 | — |

---

## 5.19 Mock 服务管理

> 在内网验证环境中，Agent 无法访问互联网的 MCP 服务器、外部 API 接口和搜索引擎。为保证 Case 测试的**可复现性和确定性**，平台提供统一的 Mock 服务管理能力，支持模拟 MCP Server、HTTP API 接口和工具调用返回，确保 Agent 在隔离环境中仍能完成完整的任务执行链路。

### 设计思路

> [!note] Mock 服务的三大核心场景
> 1. **MCP Mock**：模拟外部 MCP Server（如搜索、文件操作、数据库查询等），让 Agent 的 tool_use 调用在内网环境正常响应。
> 2. **API Mock**：模拟外部 HTTP/REST API 接口返回，覆盖 Agent 通过 HTTP 调用外部服务的场景。
> 3. **确定性控制**：通过固定 Mock 响应，消除外部服务的不确定性（如搜索结果随时间变化），保证跨芯片对比时 Agent 输入完全一致。

### 5.19.1 MCP Mock 管理

| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| MCP Mock 服务列表 | **功能说明：**<br>1. 展示所有已配置的 MCP Mock 服务<br>2. **字段：** Mock 名称、模拟的 MCP 类型（搜索 / 文件系统 / 数据库 / 代码执行 / 自定义）、工具数量、Mock 数据条目数、状态（运行中 / 已停止）、关联实验数、最近调用时间<br>3. 每行操作：启动 / 停止 / 编辑 / 复制 / 删除<br>4. 顶部 KPI：运行中 Mock 服务数、总工具数、总调用次数（今日） | — |
| 创建 MCP Mock 服务 | **功能说明：**<br>1. **基础配置：** Mock 名称（必填）、描述、模拟目标（原始 MCP Server 名称 / 地址）<br>2. **工具定义：** 逐个添加模拟的 Tool，每个 Tool 配置：<br>   a. Tool 名称（如 `web_search`、`read_file`、`query_database`）<br>   b. Tool 描述（Agent 看到的描述文本）<br>   c. 参数 Schema（JSON Schema 格式，定义入参结构）<br>   d. 响应模式：静态响应 / 模板响应 / 脚本响应 / 录制回放<br>3. **导入方式：** 支持从 MCP 协议描述文件（JSON）一键导入 Tool 定义 | — |
| Mock 响应配置 | **功能说明：**<br>1. **四种响应模式：**<br>   a. **静态响应：** 固定返回预设的 JSON 数据，适用于确定性测试<br>   b. **模板响应：** 使用 Mustache / Handlebars 模板，根据请求入参动态生成响应（如 `{{input.query}}` 映射到预设结果集）<br>   c. **脚本响应：** 编写 Python / JavaScript 脚本处理请求并返回自定义响应，支持复杂逻辑<br>   d. **录制回放：** 录制真实 MCP Server 的响应，存储为 Mock 数据集，实验时按序回放<br>2. **条件匹配：** 支持按请求参数条件匹配不同响应（如搜索关键词 A → 响应 1，关键词 B → 响应 2）<br>3. **延迟模拟：** 可配置响应延迟（固定延迟 / 随机延迟区间），模拟真实网络环境<br>4. **错误注入：** 可配置一定比例的错误响应（超时 / 500 错误 / 空结果），测试 Agent 容错能力 | — |
| MCP 响应录制 | **功能说明：**<br>1. **录制模式：** 配置真实 MCP Server 地址，平台作为代理中间人，录制所有请求-响应对<br>2. 录制产物：请求参数 + 响应内容 + 时间戳 + 耗时，存储为 JSONL 格式<br>3. 录制完成后自动生成可编辑的 Mock 数据集<br>4. 支持选择性录制：按 Tool 名称、时间范围筛选录制内容<br>5. **典型用途：** 在有互联网的环境录制真实 MCP 响应 → 导入到内网平台 → 内网实验时回放 | — |
| Mock 数据集管理 | **功能说明：**<br>1. 每个 MCP Mock 服务维护一个或多个 Mock 数据集<br>2. **数据集版本化：** 支持版本管理（v1 → v2），实验关联到具体版本<br>3. **数据条目管理：** 以请求-响应对的形式管理，支持新增、编辑、删除、批量导入<br>4. **数据格式：** 每条记录包含：请求参数（JSON）、匹配条件、响应内容（JSON）、延迟设置<br>5. **从数据资产引用：** Mock 响应内容可引用 5.18 数据资产管理中的文件（如搜索结果返回某个 PDF 的内容摘要） | — |

### 5.19.2 API Mock 管理

| 功能 | 功能详细说明 | 交互图例 |
|------|------------|---------|
| API Mock 服务列表 | **功能说明：**<br>1. 展示所有已配置的 HTTP API Mock 服务<br>2. **字段：** Mock 名称、模拟的 API 基础 URL、端点数量、状态（运行中 / 已停止）、监听端口、关联实验数<br>3. 每行操作：启动 / 停止 / 编辑 / 查看调用日志 / 删除 | — |
| 创建 API Mock | **功能说明：**<br>1. **基础配置：** Mock 名称、描述、监听端口（自动分配或手动指定）<br>2. **端点配置：** 逐个添加模拟端点：<br>   a. HTTP 方法（GET / POST / PUT / DELETE）<br>   b. 路径模式（支持路径参数，如 `/api/v1/users/:id`）<br>   c. 请求匹配条件：Header / Query Params / Body 条件<br>   d. 响应配置：状态码、响应头、响应体（JSON / XML / 纯文本）<br>3. **导入方式：**<br>   a. OpenAPI / Swagger 文件导入：自动解析端点定义，生成 Mock 配置骨架<br>   b. HAR 文件导入：从浏览器录制的 HTTP 请求中提取端点和响应<br>   c. Postman Collection 导入 | — |
| API 响应管理 | **功能说明：**<br>1. **静态响应：** 固定 JSON / XML 响应体<br>2. **动态响应：** 支持 JSONPath 表达式引用请求参数，动态构造响应<br>3. **序列响应：** 配置响应序列，第 1 次调用返回响应 A，第 2 次返回响应 B（模拟分页等场景）<br>4. **代理透传：** 未匹配的请求可配置透传到真实 API（混合模式）<br>5. **CORS 支持：** 自动处理跨域请求头 | — |
| API Mock 调用日志 | **功能说明：**<br>1. 实时记录所有命中 Mock 服务的 HTTP 请求<br>2. **字段：** 时间戳、HTTP 方法、路径、匹配的端点、响应状态码、响应耗时<br>3. 支持展开查看完整的请求头 / 请求体 / 响应头 / 响应体<br>4. 未匹配请求高亮标记，帮助发现 Mock 配置遗漏<br>5. 日志可导出为 JSONL 格式，用于分析 Agent 的 API 调用模式 | — |

### 5.19.3 Mock 服务编排与实验集成
