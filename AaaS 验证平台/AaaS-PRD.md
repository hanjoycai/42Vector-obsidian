...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 报告列表 | **功能概述：**展示所有已生成的报告，支持筛选与管理操作<br><br>**列表字段：**<br>1. 报告名称<br>2. 关联实验/Run<br>3. 生成时间<br>4. 报告类型（单实验报告 / 比对报告 / 芯片商用报告 / 周期汇总报告）<br>5. 导出状态（生成中 / 已完成 / 失败）<br>6. 文件格式<br><br>**筛选与操作：**<br>1. 支持按类型、时间、关联 Agent 筛选<br>2. 每行支持：预览、下载、重新生成、删除 | 列表 |
| 单实验验证报告 | **功能概述：**实验完成后系统自动生成（也可手动触发）<br><br>**报告结构：**<br>1. 封面与基础信息（验证时间、Agent、数据集版本、芯片/集群、模型版本、提示词版本）<br>2. 效果指标汇总（指标卡 + 雷达图）<br>3. 性能指标汇总（TTFT 分位数柱状图、延迟箱线图、吞吐量柱状图、Token 瀑布图）<br>4. 成本指标汇总（Token 消耗、¥/M-token、¥/req、¥/h + 饼图）<br>5. BadCase 清单（归因统计饼图 + Trace 链接）<br>6. 关键洞察与建议（自动生成 3～5 条优化建议）<br><br>**导出格式：**<br>1. PDF<br>2. Markdown<br>3. HTML | 报告页 |
| 芯片商用可行性报告 | **功能概述：**基于跨芯片比对数据自动生成商用可行性评估报告<br><br>**报告结构：**<br>1. 芯片性能雷达图<br>2. SLO 达标矩阵（逐指标判定）<br>3. 延迟对比图组<br>4. 成本效率对比<br>5. 算力利用率对比<br>6. 综合结论（Pass / Conditional Pass / Fail）<br><br>**审批流程：**<br>1. 可配置审批流 | 报告页 |
| 比对报告 | **功能概述：**基于多实验比对结果自动生成对比分析报告<br><br>**报告内容：**<br>1. 指标对比表<br>2. 逐 Case 差异摘要<br>3. BadCase 对比统计<br>4. 可视化图表截图：<br>a. 评分雷达对比<br>b. 延迟箱线对比<br>c. 性能散点图<br>5. 每项指标附变化率标注（较基准 +/-XX%） | 报告页 |
| 周期汇总报告 | **功能概述：**按周 / 月 / 里程碑自动汇总所有验证活动<br><br>**报告内容：**<br>1. 验证场景覆盖率<br>2. 实验次数统计<br>3. 核心指标趋势折线图<br>4. BadCase 趋势<br><br>**适用场景：**<br>1. 向管理层汇报验证进度和芯片优化效果 | 报告页 |
| 报告版本对比 | **功能概述：**同一 Agent/场景多份历史报告纵向对比<br><br>**核心能力：**<br>1. 时间轴视图展示关键指标历史变化趋势<br>2. 支持标注关键版本节点（首次验证、优化后验证、商用评审等里程碑）<br>3. 环比变化 > 15% 自动标注 | 时间轴视图 |
| 报告模板与自定义 | **功能概述：**提供内置报告模板与自定义能力<br><br>**内置模板：**<br>1. 标准验证报告<br>2. 芯片商用报告<br>3. KA 客户演示报告<br><br>**自定义能力：**<br>1. 支持自定义报告章节和图表组合<br>2. 支持添加自定义文字段落<br>3. 自定义 Logo 和页眉页脚（适用于对外输出） | 模板管理 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 平台概览 KPI 卡片 | **功能概述：**顶部指标卡片组，展示平台核心运行数据<br><br>**卡片指标：**<br>1. 运行中实验数<br>2. Runtime 总数<br>3. Total Tokens 消耗量<br>4. 异常告警数<br><br>**交互能力：**<br>1. 支持时间范围切换（近 24h / 7d / 30d）<br>2. 默认展示近 24h 数据 | 指标卡片组 |
| 调用趋势图 | **功能概述：**近 24h LLM 调用次数和工具调用次数趋势折线图<br><br>**图表配置：**<br>1. 支持按 Agent 维度下钻<br>2. 横轴按小时粒度<br>3. 纵轴为调用次数 | 折线图 |
| 告警概览 | **功能概述：**展示实时告警列表<br><br>**列表字段：**<br>1. 告警类型（模型连接超时 / KV Cache 命中率骤降 / Runtime 内存告警）<br>2. 级别（严重 / 警告 / 信息）<br>3. 触发时间<br>4. 关联模块<br><br>**操作：**<br>1. 支持一键跳转告警来源 | 告警卡片列表 |
| Token 消耗排行 | **功能概述：**展示 Top 5 模型 Token 消耗柱状图<br><br>**交互能力：**<br>1. 支持切换 Input / Output / Total Token 维度 | 柱状图 |
| 最近实验 | **功能概述：**展示最近实验活动列表<br><br>**列表字段：**<br>1. 实验名称<br>2. 关联 Agent<br>3. 运行状态<br>4. 进度条<br>5. 最近运行时间<br><br>**操作：**<br>1. 点击跳转实验详情 | 列表 |
| 导出报告快捷入口 | **功能概述：**右上角「导出报告」按钮，快速导出仪表盘数据摘要<br><br>**导出格式：**<br>1. PDF<br>2. Markdown | 按钮 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 用户列表 | **功能概述：**展示所有平台用户，支持筛选与搜索<br><br>**列表字段：**<br>1. ID<br>2. 账号（邮箱）<br>3. 用户名<br>4. 角色（7 种预置角色）<br>5. 部门<br>6. 创建人<br>7. 创建时间<br>8. 最近登录<br>9. 状态（启用 / 禁用）<br><br>**筛选与操作：**<br>1. 支持按角色 / 部门 / 状态筛选<br>2. 支持模糊搜索 | 列表 |
| 新增用户 | **功能概述：**通过表单新增平台用户<br><br>**表单字段：**<br>1. 邮箱（必填，唯一性校验）<br>2. 用户名<br>3. 角色<br>4. 部门<br>5. 初始密码（自动生成或手动）<br><br>**提交行为：**<br>1. 提交后邮件通知 | 表单弹窗 |
| 编辑用户信息 | **功能概述：**编辑已有用户的信息<br><br>**可编辑项：**<br>1. 修改基本信息<br>2. 角色重分配<br>3. 部门变更<br><br>**交互提示：**<br>1. 角色变更弹出影响范围提示 | 表单弹窗 |
| 批量操作 | **功能概述：**对多个用户执行批量管理操作<br><br>**批量能力：**<br>1. 批量重置密码<br>2. 批量启用 / 禁用<br><br>**安全机制：**<br>1. 操作前确认对话框 | 批量操作栏 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 角色列表 | **功能概述：**展示所有预置与自定义角色<br><br>**预置角色（7 种）：**<br>1. 超级管理员<br>2. 租户管理员<br>3. 验证工程师<br>4. 测试工程师<br>5. BadCase 评审员<br>6. 只读用户<br>7. 外部客户<br><br>**列表字段：**<br>1. 角色名<br>2. 描述<br>3. 当前用户数<br>4. 创建时间<br>5. 类型（预置 / 自定义）<br><br>**约束规则：**<br>1. 预置角色不可删除 | 列表 |
| 角色权限矩阵 | **功能概述：**按模块分配权限，以勾选框矩阵展示<br><br>**权限模块：**<br>1. 概览仪表盘<br>2. 资源管理（模型 / 提示词 / Agent）<br>3. 评测管理（评测集 / 评估器 / 实验）<br>4. 可观测（Trace / 会话）<br>5. 报告管理<br>6. 系统设置（用户 / 角色 / API / 标签 / 审计）<br><br>**展示形式：**<br>1. 权限矩阵勾选框展示 | 矩阵表格 |
| 自定义角色创建 | **功能概述：**创建自定义角色并分配权限<br><br>**创建表单：**<br>1. 输入角色名<br>2. 输入描述<br>3. 逐项勾选功能权限<br><br>**管理操作：**<br>1. 可编辑和删除（需先解绑关联用户） | 表单 + 权限勾选 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| MaaS 连接列表 | **功能概述：**展示所有已配置的 MaaS 连接<br><br>**列表字段：**<br>1. 名称<br>2. 类型（内部 / 外部 / 自部署）<br>3. Endpoint<br>4. 连接模型数<br>5. 数据源（Prometheus / vLLM / API / SGLang）<br>6. 最近同步时间<br>7. 健康度<br>8. 状态<br><br>**视觉规则：**<br>1. 异常红色高亮 | 列表 |
| KPI 卡片 | **功能概述：**展示 API 管理模块核心指标<br><br>**卡片指标：**<br>1. 已连接 MaaS 数<br>2. 连接模型总数<br>3. 聚合调用次数<br>4. 平均 TTFT<br><br>**趋势展示：**<br>1. 各指标较上周期变化趋势 | 指标卡片组 |
| 新建 MaaS 连接 | **功能概述：**通过配置表单新建 MaaS 连接<br><br>**表单字段：**<br>1. 连接名称<br>2. 类型<br>3. Endpoint URL<br>4. 认证（API Key / Token）<br>5. 数据源类型<br>6. 同步间隔（5min / 15min / 30min / 1h）<br><br>**提交行为：**<br>1. 提交前自动连通性测试 | 配置表单 |
| 连接健康监控 | **功能概述：**监控 MaaS 连接的健康状态<br><br>**监控能力：**<br>1. 自动健康检查（定时探测）<br>2. 手动连通性测试<br>3. 延迟监控<br><br>**告警机制：**<br>1. 连续 3 次失败自动标异常并告警 | 监控看板 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 标签分类管理 | **功能概述：**管理标签分类维度<br><br>**分类维度：**<br>1. 芯片型号（A800 / NGU800P / X6000s）<br>2. 集群标识<br>3. 场景类型<br>4. Agent 类型<br>5. 难度等级等<br><br>**操作：**<br>1. 支持增删改分类 | 分类树 |
| 标签 CRUD | **功能概述：**标签的增删改查管理<br><br>**基础操作：**<br>1. 创建标签<br>2. 编辑标签<br>3. 删除标签（删除前提示关联资源数量）<br><br>**高级操作：**<br>1. 批量创建 / 删除<br>2. 标签合并（同义标签合并，自动迁移关联） | 列表 + 表单 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 操作日志列表 | **功能概述：**记录所有用户操作和系统变更<br><br>**列表字段：**<br>1. 时间<br>2. 用户<br>3. 模块<br>4. 操作类型（创建 / 更新 / 删除 / 导出 / 登录）<br>5. 操作对象<br>6. 来源 IP<br><br>**筛选与操作：**<br>1. 支持多条件筛选<br>2. 支持全文搜索 | 列表 |
| 日志导出 | **功能概述：**将筛选后的日志数据导出<br><br>**导出能力：**<br>1. 导出为 CSV 格式<br>2. 支持选择导出时间范围 | 导出按钮 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 通知列表 | **功能概述：**展示所有通知消息<br><br>**通知类型：**<br>1. SLO 告警<br>2. 模型健康告警<br>3. 实验完成<br>4. 报告生成<br>5. 系统通知<br><br>**列表字段：**<br>1. 时间<br>2. 类型<br>3. 标题<br>4. 内容<br>5. 状态（未读 / 已读）<br><br>**视觉规则：**<br>1. 未读加粗 + 角标 | 列表 |
| 通知渠道配置 | **功能概述：**配置通知推送渠道<br><br>**推送渠道：**<br>1. 站内通知（默认）<br>2. 邮件<br>3. Webhook<br>4. 飞书<br>5. 钉钉<br><br>**配置能力：**<br>1. 每渠道独立配置<br>2. 测试通知 | 配置表单 |
| 告警规则管理 | **功能概述：**管理告警触发规则<br><br>**规则配置项：**<br>1. 监控指标<br>2. 阈值<br>3. 条件<br>4. 级别（严重 / 警告 / 信息）<br>5. 通知渠道（多选）<br><br>**管理操作：**<br>1. 规则列表支持启用/禁用 | 规则列表 + 表单 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 集群列表（只读） | **功能概述：**通过 MaaS API 同步获取所有可用集群信息<br><br>**列表字段：**<br>1. 集群名称<br>2. 集群 ID<br>3. 芯片型号（A800 / NGU800P / X6000s）<br>4. 节点数<br>5. GPU 总卡数<br>6. 可用卡数<br>7. 集群状态（在线 / 离线 / 维护中）<br>8. MaaS 来源<br>9. 最近同步时间<br><br>**筛选与同步：**<br>1. 支持按芯片型号和状态筛选<br>2. 定时自动同步（同步间隔与 API 管理模块一致） | 列表 |
| 集群详情 | **功能概述：**展示单个集群的详细信息，所有数据只读，通过 MaaS API 实时拉取<br><br>**详情内容：**<br>1. 硬件配置（芯片型号、显存规格、互联拓扑）<br>2. 已部署模型列表<br>3. 当前负载概览：<br>a. GPU 利用率<br>b. 显存使用率<br>c. 网络带宽<br>4. 关联实验列表（在该集群上运行过的实验记录） | 详情页 |
| 集群环境标签 | **功能概述：**为集群分配 AaaS 平台内的标签<br><br>**标签类型：**<br>1. 芯片型号<br>2. 集群代号<br>3. 部署环境等<br><br>**用途：**<br>1. 实验创建时选择目标环境<br>2. 跨集群比对时的分组依据<br><br>**联动：**<br>1. 标签与「标签管理」模块联动 | 标签配置 |
| 跨集群对比视图 | **功能概述：**选择 2～4 个集群，横向对比核心指标<br><br>**对比指标：**<br>1. GPU 利用率<br>2. 显存使用率<br>3. 平均 TTFT<br>4. 吞吐量（tokens/s）<br>5. 成本效率（¥/M-token）<br><br>**筛选与规则：**<br>1. 支持按时间范围筛选<br>2. 差异超过 15% 自动高亮 | 对比表格 + 柱状图 |
...
| 功能 | 详细说明 | 交互图 |
|---|---|---|
| 资产总览与文件管理 | **功能概述：**统一管理平台数据资产，支持多格式上传与检索<br><br>**KPI 卡片：**<br>1. 资产总数<br>2. 存储占用<br>3. 引用数<br><br>**支持格式：**<br>1. PDF / DOCX / XLSX<br>2. Markdown<br>3. 代码仓库<br>4. JSON / CSV<br>5. 图片<br>6. 音视频<br><br>**核心能力：**<br>1. 自动计算 SHA256 哈希<br>2. 目录树组织 + 全文检索 | 列表 + 目录树 |
| Agent 知识库 | **功能概述：**面向 Agent 场景的知识库管理<br><br>**分组维度：**<br>1. 产品文档<br>2. 代码仓库<br>3. FAQ 语料<br>4. 多轮对话样本<br><br>**索引能力：**<br>1. 支持文档索引（全文索引 + 向量索引）<br>2. 索引状态追踪（已索引 / 索引中 / 待索引）<br><br>**筛选与操作：**<br>1. 支持搜索和分组筛选 | 分组列表 |
| 多模态素材库 | **功能概述：**管理多模态素材资源<br><br>**支持素材类型：**<br>1. 图像（PNG/JPG/HEIC/SVG）<br>2. 音频（MP3/WAV/FLAC）<br>3. 视频（MP4/MOV/WebM）<br>4. PDF<br><br>**展示字段：**<br>1. 素材格式<br>2. 引用次数<br>3. 大小<br><br>**筛选与操作：**<br>1. 支持按素材类型筛选<br>2. 支持关键词搜索 | 分组列表 |
| 资产版本管理 | **功能概述：**管理资产的版本历史<br><br>**版本规则：**<br>1. 同一资产上传新版本自动递增版本号<br>2. 历史版本不可删除保证追溯<br>3. 评测集 Case 引用时锁定到具体版本号 | 版本列表 |
| 资产引用与沙箱挂载 | **功能概述：**管理资产与 Case/实验的引用关系<br><br>**核心能力：**<br>1. Case 可从资产库选择文件作为 attachments<br>2. 引用追踪（反向关联列表）<br>3. 实验运行时关联资产自动挂载到 Agent 沙箱 | 关联配置 |
| 代码仓库快照 | **功能概述：**Coding Agent 场景专用，管理代码仓库快照<br><br>**核心能力：**<br>1. 上传代码仓库快照（git archive / zip）<br>2. 绑定 commit hash<br>3. 绑定 Docker 镜像<br>4. 绑定 F2P/P2P 测试集<br>5. 跨芯片一致性保证 | 详情页 |
